...
//
//   System      : MESClient
//   File Name   : frmInputAuditPlan.cs
//   Description : 
//
...
{
    public partial class frmInputAuditPlan : ViewForm01
    {
        public frmInputAuditPlan()
        {
            InitializeComponent();
        }
        ...
        {
            TRSNode in_node = new TRSNode("CAMS_VIEW_AUDIT_RESULT_LIST_IN");
        ...
        ////       -
        ////
        private bool DeleteAuditPlan()
        {
            TRSNode in_node = new TRSNode("CAMS_UPDATE_AUDIT_PLAN_IN");
            TRSNode out_node = new TRSNode("CAMS_UPDATE_AUDIT_PLAN_OUT");
            TRSNode list_item;
            bool b_save_flag = false;
            try
            {
                MPCR.SetInMsg(in_node);
                in_node.ProcStep = MPGC.MP_STEP_DELETE; //DELETE∏∏ ¿÷¿Ω.
                in_node.AddChar("LIST_FLAG", 'Y');

                FarPoint.Win.Spread.SheetView with_1 = spdAuditList.ActiveSheet;

                for (int i = 0; i < with_1.RowCount; i++)
                {
                    if (Convert.ToBoolean(with_1.Cells[i, (int)PLAN_LIST.CHECK].Value) == true)
                    {
                        if (MPCF.ToChar(with_1.GetValue(i, (int)PLAN_LIST.STATUS)) != '0')
                        {
                            MPCF.ShowMsgBox(MPCF.GetMessage(636)); //Only standby status can be deleted.
                            with_1.SetActiveCell(i, (int)PLAN_LIST.AUDIT_ID);
                            return false;
                        }
                        b_save_flag = true;
                        list_item = in_node.AddNode("AUDIT_DELETE_LIST");
                        list_item.AddString("AUDIT_ID", MPCF.Trim(with_1.Cells[i, (int)PLAN_LIST.AUDIT_ID].Value));
                    }
                }

                if (b_save_flag == false)
                {
                    MPCF.ShowMsgBox(MPCF.GetMessage(633)); //No data to delete.
                    return false;
                }

                if (MPCF.ShowMsgBox(MPCF.GetMessage(367), MessageBoxButtons.YesNo, 2) != System.Windows.Forms.DialogResult.Yes)
                {
                    return false;
                }
        ...
        private bool OpenAuditPlanPopup(bool b_new_flag, string s_audit_id)
        {
            try
            {
                frmInputAuditPlanPopup _resultPopup = new frmInputAuditPlanPopup();
                _resultPopup.NEW_FLAG = b_new_flag;
        ...
        private void frmInputAuditPlan_Activated(object sender, System.EventArgs e)
        {
            ...
            try
            {
                switch (e.Column)
                {
                    case (int)PLAN_LIST.DETAIL:
                        ...
                        OpenAuditPlanPopup(false, MPCF.Trim(spdAuditList.ActiveSheet.GetValue(e.Row, (int)PLAN_LIST.AUDIT_ID)));
                        break;
                }
            }
            catch (Exception ex)
