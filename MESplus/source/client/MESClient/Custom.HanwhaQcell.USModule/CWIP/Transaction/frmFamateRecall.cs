...
{
    public partial class frmFamateRecall : Form
    {
        Dictionary<String, XmlNode> smgMaster, hmgMaster;

        Dictionary<string, string> m = new Dictionary<string, string>();

        public frmFamateRecall()
        {
            InitializeComponent();
        ...
        private void Process(string type, string[] fileNames)
        {            
            ...
            foreach (string filename in fileNames)
            {
                    ...
                    for (int x = 0; x < smgList.Count; x++)
                    {
                        ...
                        if (Attribute(smg, "N").Equals("InspectionResultReportSend"))
                        {
                            ...
                            if (eqpid.Contains("-AOI-") || eqpid.Contains("-MVI-"))
                            {
                                ...
                                buffer += "INSERT INTO CEDCLOTRLH_TEMP SELECT * FROM CEDCLOTRLH WHERE INS_CNT = 1 AND ROWNUM = 1 AND LOT_ID = '" + moduleid + "' ORDER BY INS_TIME;\r\n";
                ...
                else if (type.Equals("TACK"))
                {
                    XmlNodeList hmgList = doc.GetElementsByTagName("HMG");
                    for (int x = 0; x < hmgList.Count; x++)
                    {
                        XmlNode hmg = hmgList[x];
                        if (Attribute(hmg, "N").Equals("EAPMES_Get_Process_Tack_Time"))
                        {
                            string resId = Attribute(hmg.SelectSingleNode(".//HIT[@N='RES_ID']"), "V");
                            string orgMsgId = Attribute(hmg.SelectSingleNode(".//HIT[@N='ORG_MSG_ID']"), "V");
                            string clientTime = Attribute(hmg.SelectSingleNode(".//HIT[@N='CLIENT_TIME']"), "V");
                            string lineId = Attribute(hmg.SelectSingleNode(".//HIT[@N='LINE_ID']"), "V");
                            string tackTime = Attribute(hmg.SelectSingleNode(".//HIT[@N='TACK_TIME']"), "V");
                            string locId = Attribute(hmg.SelectSingleNode(".//HIT[@N='LOC_ID']"), "V");

                            buffer += resId + "\t" + locId + "\t" + clientTime + "\t" + orgMsgId + "\t" + tackTime + "\r\n";
                        }
                        fileProgressBar.Invoke(new UpdateFileProgressBar(UpdateFileProgressBar_Process), (int)((double)(x + 1) / (double)hmgList.Count * 100));
                    }
                }
                    ...
                    for (int x = 0; x < hmgList.Count; x++)
                    {
                        ...
                        if (Attribute(hmg, "N").Equals("EAPMES_Collect_Inspection_Data_Sualab"))
                        {
                            ...
                            if (eqpid.Contains("-FEL-"))
                            {
                                ...
                                buffer += "UPDATE MWIPLOTHIS SET TRAN_TIME = '" + gemclock.Substring(0, 14) + "' WHERE LOT_ID = '" + moduleid + "' AND OPER ='M2040' AND TRAN_CODE = 'CREATE';\r\n";
                                buffer += "UPDATE MWIPLOTHIS SET TRAN_TIME = '" +gemclock.Substring(0,14) + "' WHERE LOT_ID = '" + moduleid + "' AND OPER ='M2040' AND TRAN_CODE = 'START';\r\n";
                                buffer += "UPDATE MWIPLOTHIS SET TRAN_TIME = '" + gemclock.Substring(0, 14) + "' WHERE LOT_ID = '" + moduleid + "' AND OPER ='M3000' AND TRAN_CODE = 'END';\r\n";
                                buffer += "UPDATE MWIPLOTSTS SET CREATE_TIME = '" + gemclock.Substring(0, 14) + "', LAST_TRAN_TIME = '" + gemclock.Substring(0, 14) + "' WHERE LOT_ID = '" + moduleid + "' AND OPER ='M3000';\r\n";
                                buffer += "UPDATE MWIPLOTHIS SET SYS_TRAN_TIME = TRAN_TIME, START_TIME = TRAN_TIME, END_TIME = TRAN_TIME, CREATE_TIME = TRAN_TIME, FAC_IN_TIME = TRAN_TIME, FLOW_IN_TIME = TRAN_TIME, OPER_IN_TIME = TRAN_TIME, OLD_FAC_IN_TIME = TRAN_TIME, OLD_FLOW_IN_TIME = TRAN_TIME, OLD_OPER_IN_TIME = TRAN_TIME WHERE LOT_ID = '" + moduleid + "';\r\n";

                                buffer += "UPDATE CEDCLOTRLH SET INS_TIME = '" + gemclock.Substring(0, 14) + "',RESULT_TIME = '" + gemclock.Substring(0, 14) + "', CMF_1 = '" + gemclock.Substring(0, 14) + "' WHERE INS_TYPE = 'E1' AND LOT_ID = '" + moduleid + "';\r\n";
                                buffer += "UPDATE CEDCLOTRLT SET INS_TIME = '" + gemclock.Substring(0, 14) + "',RESULT_TIME = '" + gemclock.Substring(0, 14) + "', CMF_1 = '" + gemclock.Substring(0, 14) + "' WHERE INS_TYPE = 'E1' AND LOT_ID = '" + moduleid + "';\r\n";
                                

                                //buffer += "UPDATE MWIPLOTHIS_TEMP SET TRAN_TIME = '" + gemclock.Substring(0, 14) + "', OLD_OPER = 'M2040', OPER = 'M2040', START_RES_ID = '" + eqpid + "', END_RES_ID = ' ', PREV_ACTIVE_HIST_SEQ = -1, HIST_SEQ = (SELECT DECODE(MIN(HIST_SEQ), 1, 0, MIN(HIST_SEQ))-6 FROM MWIPLOTHIS WHERE LOT_ID = '" + moduleid + "') WHERE TRAN_CODE = 'START' AND LOT_ID = '" + moduleid + "';\r\n";
                                //buffer += "UPDATE MWIPLOTHIS_TEMP SET TRAN_TIME = '" + gemclock.Substring(0, 14) + "', OLD_OPER = 'M2040', OPER = 'M3000', START_RES_ID = '" + eqpid + "', END_RES_ID = '" + eqpid + "', PREV_ACTIVE_HIST_SEQ = -1, HIST_SEQ = (SELECT DECODE(MIN(HIST_SEQ), 1, 0, MIN(HIST_SEQ))-5 FROM MWIPLOTHIS WHERE LOT_ID = '" + moduleid + "') WHERE TRAN_CODE = 'END' AND LOT_ID = '" + moduleid + "';\r\n";
                                //buffer += "UPDATE MWIPLOTHIS_TEMP SET SYS_TRAN_TIME = TRAN_TIME, START_TIME = TRAN_TIME, END_TIME = TRAN_TIME, CREATE_TIME = TRAN_TIME, FAC_IN_TIME = TRAN_TIME, FLOW_IN_TIME = TRAN_TIME, OPER_IN_TIME = TRAN_TIME, OLD_FAC_IN_TIME = TRAN_TIME, OLD_FLOW_IN_TIME = TRAN_TIME, OLD_OPER_IN_TIME = TRAN_TIME WHERE LOT_ID = '" + moduleid + "';\r\n";
                                //buffer += "INSERT INTO CEDCLOTRLH_TEMP SELECT * FROM CEDCLOTRLH WHERE INS_CNT = 1 AND ROWNUM = 1 AND LOT_ID = '" + moduleid + "' ORDER BY INS_TIME;\r\n";
                ...
                else if (type.Equals("FELCALL"))
                {
                    ...
                    for (int x = 0; x < hmgList.Count; x++)
                    {
                        ...
                        if (Attribute(hmg, "N").Equals("EAPMES_Start_Lot"))
                        {
                            string eqpid = Attribute(hmg.SelectSingleNode(".//HIT[@N='RES_ID']"), "V");
                            if (eqpid.Contains("-FEL-"))
                            {
                                ...
                                string gemclock = Attribute(hmg.SelectSingleNode(".//HIT[@N='CLIENT_TIME']"), "V");
                                string lineid = Attribute(hmg.SelectSingleNode(".//HIT[@N='LINE_ID']"), "V");
                                string clientID = Attribute(hmg.SelectSingleNode(".//HIT[@N='CLIENT_ID']"), "V");
                                if (!inputList.Contains(lotId) && inputList.Count > 0) continue;
                                ...
                                in_node.ProcStep = '1';
                                in_node.SetString("CLIENT_TIME", gemclock);
                                in_node.SetString("CLIENT_ID", clientID);
                                in_node.SetString("RES_ID", eqpid);
                                in_node.SetString("LOT_ID", lotId);
                                in_node.SetString("LINE_ID", lineid);

                                if (MPCR.CallService("CEIS", "EAPMES_Start_Lot", in_node, ref out_node) == false)
                                {
                                    continue;
                                }

                                buffer += "[LOT START]" + lotId + "\r\n";
                            }
                        }
                        ...
                        if (Attribute(hmg, "N").Equals("EAPMES_Collect_Inspection_Data_Sualab"))
                        {
                            ...
                            if (eqpid.Contains("-FEL-"))
                            {
                                ...
                                string mode = Attribute(hmg.SelectSingleNode(".//HIT[@N='MODE']"), "V");
                                string clientID = Attribute(hmg.SelectSingleNode(".//HIT[@N='CLIENT_ID']"), "V");
                                string moduleImage = Attribute(hmg.SelectSingleNode(".//HIT[@N='MODULE_IMAGE']"), "V");
                                string productInfo = Attribute(hmg.SelectSingleNode(".//HIT[@N='PRODUCT_INFO']"), "V");
                                string userId = Attribute(hmg.SelectSingleNode(".//HIT[@N='USER_ID']"), "V");
                                if (!inputList.Contains(lotId) && inputList.Count > 0) continue;

                                TRSNode in_node = new TRSNode("CMN_IN");
                                TRSNode out_node = new TRSNode("CMN_OUT");
                                TRSNode inspection_list = in_node.AddNode("INSPECTION_LIST");
                                TRSNode inspection_item;
                                ...
                                in_node.SetString("RES_ID", eqpid);
                                in_node.SetString("LOT_ID", lotId);
                                in_node.SetString("RESULT", result);
                                in_node.SetString("MODE", mode);
                                in_node.SetString("MODULE_IMAGE", moduleImage);
                                in_node.SetString("PRODUCT_INFO", productInfo);
                                in_node.SetString("USER_ID", userId);
                                ...
                                if (paramlist != null)
                                {
                                    foreach (XmlNode param in paramlist.ChildNodes)
                                    {
                                        inspection_item = inspection_list.AddNode("INSPECTION_ITEM");
                                        inspection_item.SetString("AOI_LOCATION", Attribute(param.ParentNode.SelectSingleNode(".//HIT[@N='AOI_LOCATION']"), "V"));
                                        inspection_item.SetString("AOI_REASON_CODE", Attribute(param.ParentNode.SelectSingleNode(".//HIT[@N='AOI_REASON_CODE']"), "V"));
                                        inspection_item.SetString("ML_LOCATION", Attribute(param.ParentNode.SelectSingleNode(".//HIT[@N='ML_LOCATION']"), "V"));
                                        inspection_item.SetString("ML_REASON_CODE", Attribute(param.ParentNode.SelectSingleNode(".//HIT[@N='ML_REASON_CODE']"), "V"));
                                        inspection_item.SetString("INSPECTION_RESULT", Attribute(param.ParentNode.SelectSingleNode(".//HIT[@N='INSPECTION_RESULT']"), "V"));
                                        inspection_item.SetString("CELL_IMAGE", Attribute(param.ParentNode.SelectSingleNode(".//HIT[@N='CELL_IMAGE']"), "V"));
                                        inspection_item.SetString("AOI_IMAGE", Attribute(param.ParentNode.SelectSingleNode(".//HIT[@N='AOI_IMAGE']"), "V"));
                                    }
                                }

                                if (MPCR.CallService("CEIS", "EAPMES_Collect_Inspection_Data_Sualab", in_node, ref out_node) == false)
                                {
                                    continue;
                                }

                                buffer += "[LOT INSPECTION]" + lotId + "\r\n";


                            }
                        }

                        if (Attribute(hmg, "N").Equals("EAPMES_End_Lot"))
                        {
                            string eqpid = Attribute(hmg.SelectSingleNode(".//HIT[@N='RES_ID']"), "V");
                            if (eqpid.Contains("-FEL-"))
                            {
                                ...
                                if (MPCR.CallService("CEIS", "EAPMES_End_Lot", in_node, ref out_node) == false)
                                {
                                    continue;
                                }

                                buffer += "[LOT END]" + lotId + "\r\n";
                            }
                        }
                    ...
                    for (int x = 0; x < hmgList.Count; x++)
                    {
                        ...
                        if (Attribute(hmg, "N").Equals("EAPMES_Collect_Inspection_Data"))
                        {
                            ...
                            if (eqpid.Contains("-SIM-"))
                            {
                                ...
                                string clientID = Attribute(hmg.SelectSingleNode(".//HIT[@N='CLIENT_ID']"), "V");
                                string moduleImage = Attribute(hmg.SelectSingleNode(".//HIT[@N='MODULE_IMAGE']"), "V");
                                string reasonCode = Attribute(hmg.SelectSingleNode(".//HIT[@N='REASON_CODE']"), "V");
                                string userId = Attribute(hmg.SelectSingleNode(".//HIT[@N='USER_ID']"), "V");
                                if (!inputList.Contains(lotId) && inputList.Count > 0) continue;

                                TRSNode in_node = new TRSNode("CMN_IN");
                                TRSNode out_node = new TRSNode("CMN_OUT");
                                TRSNode defect_list = in_node.AddNode("DEFECT_LIST");

                                TRSNode param_list = in_node.AddNode("PARAM_LIST");
                                TRSNode param_item;
                                ...
                                in_node.SetString("RESULT", result);
                                in_node.SetString("MODULE_IMAGE", moduleImage);
                                in_node.SetString("REASON_CODE", reasonCode);
                                in_node.SetString("USER_ID", userId);
                                ...
                                if (paramlist != null)
                                {
                                    foreach (XmlNode param in paramlist.ChildNodes)
                                    {
                                        param_item = param_list.AddNode("PARAM_ITEM");
                                        param_item.AddString("PARAM_NAME", Attribute(param.SelectSingleNode(".//HIT[@N='PARAM_NAME']"), "V"));
                                        param_item.AddString("PARAM_VALUE", Attribute(param.SelectSingleNode(".//HIT[@N='PARAM_VALUE']"), "V"));                                                                                                         
                                    }
                                }

                                if (MPCR.CallService("CEIS", "EAPMES_Collect_Inspection_Data", in_node, ref out_node) == false)
                                {
                                    continue;
                                }
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (Control radioButton in flowLayoutPanel2.Controls)
                {
                    if (radioButton is RadioButton && ((RadioButton)radioButton).Checked)
                    {
                        ...
                        Process(radioButton.Tag.ToString(), openFileDialog1.FileNames);
                        return;
                    }
                }
            }            
