...
namespace SOI.OIFrx
{
    ...
    public class clsFlexibleScreenInputValue
    {
        ...
        {
            if (MPOF.Trim(s_member_name) == "")
            {
                MPOF.ShowMsgBox("No Member Named for input value on flexible screen.");
                return null;
            }

            if (ml_cells == null)
            {
                MPOF.ShowMsgBox("Empty cell information.");
                return null;
            }
            ...
            MPOF.ShowMsgBox("No have Member in Cell List.");
            return null;
        }
        ...
        public string GetName(int index)
        {
            ...
            {
                MPOF.ShowMsgBox("Out of range for the cells list.");
                return null;
            }
        ...
        public string[] GetComboItems(string s_member_name)
        {
            ...
            {
                if (MPOF.Trim(cell.items[i].desc) == "")
                {
            ...
            for (i = 0; i < ml_cells.Count; i++)
            {
                switch (ml_cells[i].cell_type)
                {
                    ...
                    case FSCREEN_CELL_TYPE.FLOAT:
                        if (ml_cells[i].input_value != null && MPOF.Trim(ml_cells[i].input_value) != "")
                        {
                    ...
                    case FSCREEN_CELL_TYPE.COMBOBOX:
                        if (ml_cells[i].input_value != null && MPOF.Trim(ml_cells[i].input_value) != "")
                        {
                            ...
                            node.AddString(ml_cells[i].member_name, MPOF.Trim(s_temp[0]));
                        }
                        break;
                    ...
                    case FSCREEN_CELL_TYPE.HYPERLINK:
                        if (ml_cells[i].input_value != null && i_bin_data_count <= 10)
                        {
                            ...
                            s_temp = MOGC.MP_BIN_DATA_1.Substring(0, MOGC.MP_BIN_DATA_1.Length - 1);
