...
namespace SOI.OIFrx
{
    public partial class udcCustomCollectDataQc : UserControl
    {
        public udcCustomCollectDataQc()
        {
            InitializeComponent();
            Init();
        }
        ...
        private const int VALUE_START_COL = 33;
        private const int DEFAULT_COL_COUNT = 33;
        ...
        public string SpecSaveFlag
        {
            get
            {
                return ms_spec_save_flag;
            }
            set
            {
                ms_spec_save_flag = value;
            }
        }
        ...
        private string ms_spec_save_flag;
            ...
            try
            {
                ...
                for (i = 0; i < spdData.ActiveSheet.RowCount; i++)
                {
                    if (MPCF.ToInt(spdData.ActiveSheet.GetValue(i, MPCF.ToInt(CHAR_COLUMN.UNIT_COUNT))) == 1)
                    {
                        ...
                        {
                            char_item.AddString("INSP_COMMENT", MPCF.Trim(spdData.ActiveSheet.GetValue(i, iSpdColCount + 5)));
                        }
                        ...
                        if (SpecSaveFlag == "Y")
                        {
                            char_item.AddString("MIN_VALUE", MPCF.Trim(spdData.ActiveSheet.GetValue(i, MPCF.ToInt(CHAR_COLUMN.MST_LOWER_LIMIT))));
                            char_item.AddString("MAX_VALUE", MPCF.Trim(spdData.ActiveSheet.GetValue(i, MPCF.ToInt(CHAR_COLUMN.MST_MAX_LIMIT))));
                            char_item.AddString("STD_VALUE", MPCF.Trim(spdData.ActiveSheet.GetValue(i, MPCF.ToInt(CHAR_COLUMN.STD_VALUE))));
                        }
                    ...
                    for (int i = 0; i < spdData.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (spdData.Sheets[0].Cells[i, (int)CHAR_COLUMN.VALUE_TYPE_COL].Text == "N")
                        {
                            ...
                            if (iCount > 0)
                            {
                                ...
                                spdData.Sheets[0].Cells[i, iSpdColCount + 1].Text = MPCF.Trim(iDev);
                            }
                        }
        ...
        private void spdData_EditModeOff(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    if (iColumn == (int)CHAR_COLUMN.MST_LOWER_LIMIT || iColumn == (int)CHAR_COLUMN.MST_MAX_LIMIT || iColumn == (int)CHAR_COLUMN.STD_VALUE)
                    {
                        CheckByCell_1(spdData_Sheet1, iRow, iColumn);
                    }
                    else
                    {
                        //Spec out 색깔 변경
                ...
                if (control is FpSpread)
                {
                    ...
                    do
                    {
                        ...
                        for (i = 0; i < out_node.GetList(0).Count; i++)
                        {
                            ...
                            if (out_node.GetList(0)[i].GetChar("VALUE_TYPE") == 'A')
                            {
                                with_1.Cells[i, (int)CHAR_COLUMN.STD_VALUE].Locked = true;
                            }
                            ...
                            with_1.Cells[i, (int)CHAR_COLUMN.VALUE_RESULT_COL].BackColor = System.Drawing.Color.WhiteSmoke;

                            with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.MST_LOWER_LIMIT), out_node.GetList(0)[i].GetString("LOWER_CUST_LIMIT"));
                            with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.MST_MAX_LIMIT), out_node.GetList(0)[i].GetString("UPPER_CUST_LIMIT"));
                            ...
                            if (CommentViewFlag == "Y")
                            {
                                with_1.ColumnHeader.Cells[0, iColCnt + 5].Text = "Comment";
                            ...
                            if (AbsValueViewFlag == "Y")
                            {
                                with_1.ColumnHeader.Cells[0, iColCnt + 1].Text = "편차";
                ...
                if (control is FpSpread)
                {
                    ...
                    do
                    {
                        ...
                        for (i = 0; i < out_node.GetList(0).Count; i++)
                        {
                            ...
                            with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.DEF_UNIT_OVR_FLAG_COL), out_node.GetList(0)[i].GetChar("DEF_UNIT_OVR_FLAG"));
                            with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.LOWER_LIMIT), out_node.GetList(0)[i].GetString("LOWER_SPEC_LIMIT"));
                            with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.STD_VALUE), out_node.GetList(0)[i].GetString("STD_VALUE"));
                            with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.MST_LOWER_LIMIT), out_node.GetList(0)[i].GetString("MIN_VALUE"));
                            with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.MST_MAX_LIMIT), out_node.GetList(0)[i].GetString("MAX_VALUE"));
                            ...
                            if (AbsValueViewFlag == "Y")
                            {
                                txtCellType = (FarPoint.Win.Spread.CellType.TextCellType)(with_1.Cells[i, iColCnt + 1].CellType);
                                with_1.Columns[iColCnt + 1].Width = 50;
                                with_1.Cells[i, iColCnt + 1].Text = sAbsValue;
                                with_1.ColumnHeader.Cells[0, iColCnt + 1].Text = "편차";
                                with_1.AddColumnHeaderSpanCell(0, iColCnt + 1, 2, 1);
                            }
                            else
                            ...
                            {
                                txtCellType = (FarPoint.Win.Spread.CellType.TextCellType)(with_1.Cells[i, iColCnt + 5].CellType);
                                with_1.Columns[iColCnt + 5].Width = 200;
                                with_1.Cells[i, iColCnt + 5].Text = sInspComment;
                                with_1.ColumnHeader.Cells[0, iColCnt + 5].Text = "Comment";
                                with_1.AddColumnHeaderSpanCell(0, iColCnt + 5, 2, 1);
                            }
                            else
                            ...
                            if (SpecSaveFlag == "Y" && cUnitType == 'N')
                            {
                                with_1.Cells[i, (int)CHAR_COLUMN.STD_VALUE].Locked = false;
                                with_1.Cells[i, (int)CHAR_COLUMN.MST_LOWER_LIMIT].Locked = false;
                                with_1.Cells[i, (int)CHAR_COLUMN.MST_MAX_LIMIT].Locked = false;

                                with_1.Cells[i, (int)CHAR_COLUMN.STD_VALUE].BackColor = Color.White;
                                with_1.Cells[i, (int)CHAR_COLUMN.MST_LOWER_LIMIT].BackColor = Color.White;
                                with_1.Cells[i, (int)CHAR_COLUMN.MST_MAX_LIMIT].BackColor = Color.White;
                            }
                        }
        ...
        private void CheckByCell(FarPoint.Win.Spread.SheetView spdView, int nRow, int nCol)
        {
            ...
            try
            {
                ...
                {
                    dTargetValue = MPCF.ToDbl(spdView.Cells[nRow, (int)CHAR_COLUMN.STD_VALUE].Text);
                    dValue = MPCF.ToDbl(spdView.Cells[nRow, nCol].Text);

                    sLowerLimit = spdView.Cells[nRow, (int)CHAR_COLUMN.MST_LOWER_LIMIT].Text;
                    sUpperLimit = spdView.Cells[nRow, (int)CHAR_COLUMN.MST_MAX_LIMIT].Text;

                    dLowerLimit = dTargetValue + MPCF.ToDbl(sLowerLimit);
                    dUpperLimit = dTargetValue + MPCF.ToDbl(sUpperLimit);
        ...
        private void CheckByCell_1(FarPoint.Win.Spread.SheetView spdView, int nRow, int nCol)
        {
            string sValueType = string.Empty;
            ...
            try
            {
                ...
                {
                    int iRowCount = 0;
                    iRowCount = MPCF.ToInt(spdView.Cells[nRow, (int)CHAR_COLUMN.SAMPLE_SIZE].Text);

                    for (int i = 0; i < iRowCount; i++)
                    {
                        if (string.IsNullOrEmpty(spdView.Cells[nRow, DEFAULT_COL_COUNT + i].Text) == false)
                        {
                            dTargetValue = MPCF.ToDbl(spdView.Cells[nRow, (int)CHAR_COLUMN.STD_VALUE].Text);
                            dValue = MPCF.ToDbl(spdView.Cells[nRow, DEFAULT_COL_COUNT + i].Text);
                            ...
                            if ((MPCF.Trim(sLowerLimit) != string.Empty && dValue < dLowerLimit) ||
                                 (MPCF.Trim(sUpperLimit) != string.Empty && dValue > dUpperLimit))
                            {
                                spdView.Cells[nRow, DEFAULT_COL_COUNT + i].BackColor = Color.Red;
                                spdView.Cells[nRow, DEFAULT_COL_COUNT + i].ForeColor = Color.PaleTurquoise;
                            }
                            else
                            {
                                spdView.Cells[nRow, DEFAULT_COL_COUNT + i].BackColor = Color.White;
                                spdView.Cells[nRow, DEFAULT_COL_COUNT + i].ForeColor = Color.Black;
                            }
