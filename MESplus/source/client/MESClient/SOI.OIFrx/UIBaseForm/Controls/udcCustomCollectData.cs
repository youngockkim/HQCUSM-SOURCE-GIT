...
namespace SOI.OIFrx
{
    public partial class udcCustomCollectData : UserControl
    {
        public udcCustomCollectData()
        {
            InitializeComponent();
            Init();
        }
            ...
            try
            {
                ...
                for (i = 0; i < spdData.ActiveSheet.RowCount; i++)
                {
                    if (MPCF.ToInt(spdData.ActiveSheet.GetValue(i, MPCF.ToInt(CHAR_COLUMN.UNIT_COUNT))) == 1)
                    {
                        ...
                        if (SpecSaveFlag == "Y")
                        {
                            char_item.AddString("MIN_VALUE", MPCF.Trim(spdData.ActiveSheet.GetValue(i, MPCF.ToInt(CHAR_COLUMN.LOWER_LIMIT))));
                            char_item.AddString("MAX_VALUE", MPCF.Trim(spdData.ActiveSheet.GetValue(i, MPCF.ToInt(CHAR_COLUMN.MAX_LIMIT))));
                        }
                ...
                if (control is FpSpread)
                {
                    ...
                    do
                    {
                        ...
                        for (i = 0; i < out_node.GetList(0).Count; i++)
                        {
                            ...
                            with_1.Cells[i, MPCF.ToInt(CHAR_COLUMN.CHAR_DESC_COL)].CellType = txtCellType1;
                ...
                if (control is FpSpread)
                {
                    ...
                    do
                    {
                        ...
                        for (i = 0; i < out_node.GetList(0).Count; i++)
                        {
                            ...
                            if (SpecSaveFlag == "Y")
                            {
                                with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.LOWER_LIMIT), out_node.GetList(0)[i].GetString("MIN_VALUE"));
                                with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.MAX_LIMIT), out_node.GetList(0)[i].GetString("MAX_VALUE"));
                            }
                            else
                            {
                                with_1.SetValue(i, MPCF.ToInt(CHAR_COLUMN.LOWER_LIMIT), out_node.GetList(0)[i].GetString("LOWER_SPEC_LIMIT"));
