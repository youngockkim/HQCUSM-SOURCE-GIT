...
{
    public partial class frmCodeViewPopup_Mat : frmPopupBase
    {
        #region Property
        ...
        private CodeViewPopupModel_Mat cvpm;
        ...
        /// <param name="e"></param>
        private void frmCodeViewPopup_Mat_Load(object sender, EventArgs e)
        {
            // Title 변경
            lblPopupTitle.Text = MPCF.FindLanguage(cvpm.Title);
        ...
        private void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                MPCF.ShowErrorMessage("CodeView btnRefresh_Click() \n" + ex.Message);
            }
        }
        ...
        /// <returns></returns>
        public bool InitCodeViewPopup(BOIMatCodeView control, string popupTitle, string moduleName, string serviceName, TRSNode inNode,
            string listName, string[] displayColumn, string[] header, string selectedColumn, int pageSize)
        {
            // 생성
            if (cvpm == null)
            {
                cvpm = new CodeViewPopupModel_Mat();
            }
        ...
        public bool InitCodeViewPopup(BOIMatCodeView control, string popupTitle, string ViewID, TPDR.DirectViewCond[] cvcArgu,
        ...
        private bool ViewService()
        {
            ...
            try
            {
                if (!cvpm.isTable)
                {
                    ...
                    // Service Call
                    if (MPCF.CallService(cvpm.ModuleName, cvpm.ServiceName, cvpm.InNode, ref out_node) == false)
                    {
                        // Error Message
                ...
                else
                {
                    ...
                    if (TPDR.GetDataOne("", ref dt, cvpm.ViewID, cvpm.Argu, false, false, ref sSql) == false)
                    //if (TPDR.DirectViewOne(gridCodeViewPopup, cvpm.ViewID, ref dt, false, false, cvpm.Argu, false, false) == false)
                    {
                        if (dt != null)
                            dt.Dispose();
                    ...
                    if (cvpm.Header == null)
                    {
                        ...
                        {
                            cboFind.Items.Add(dt.Columns[i].ColumnName.Replace(' ', '_'), dt.Columns[i].ColumnName);
                            dt.Columns[i].ColumnName = dt.Columns[i].ColumnName.Replace(' ', '_');
                        }

                        gridCodeViewPopup.SetDataSource(dt);

                        for (i = 0; i < dt.Columns.Count; i++)
                        {
                            gridCodeViewPopup.DisplayLayout.Bands[0].Columns[i].Header.Caption = MPCF.FindLanguage(dt.Columns[i].ColumnName.Replace('_', ' '));
                        }
                    ...
                    {
                        // Data Bind
                        dtOriginalData = BindToSOIGrid(gridCodeViewPopup, dt, cvpm.DisplayColumn, cvpm.Header);
                    }
                }

                // Scroll
                MPCF.SetScrollRow(gridCodeViewPopup, cvpm.SelectedColumn, cvpm.Control.Text);
        ...
        private bool ViewPageService(int pageIndex, int pageSize)
        {
            try
            {
                ...
                if (MPCF.CallService(cvpm.ModuleName, cvpm.ServiceName, cvpm.InNode, ref out_node) == false)
                {
                    ...
                    if (string.IsNullOrEmpty(out_node.GetString("MSG")) == false)
                    {
                        MPCF.ShowMessage(out_node.GetString("MSG"), CliFrx.MSG_LEVEL.ERROR, true);
                    }
        ...
        private void InitFindComboBox()
        {
            ...
            for (int i = 0; i < cvpm.DisplayColumn.Length; i++)
            {
                ...
                {
                    cboFind.Items.Add(cvpm.DisplayColumn[i], MPCF.FindLanguage(cvpm.DisplayColumn[i]));
                }
                // Header를 사용한 경우
                else
                {
                    cboFind.Items.Add(cvpm.DisplayColumn[i], MPCF.FindLanguage(cvpm.Header[i])); 
                }
            }
        ...
        private void FindData()
        {
            try
            {
                ...
                if (dtOriginalData != null)
                {
                    ...
                    // Bind
                    if (cvpm.DisplayColumn == null)
                        BindToSOIGrid(gridCodeViewPopup, dtOriginalData.DefaultView.ToTable());
                    else
            ...
            {
                MPCF.ShowErrorMessage("CodeView FindData() \n" + ex.Message);
            }
        }
        ...
        private int GetIndexDisplayColumn(string columnName)
        {
            ...
            if (cvpm.DisplayColumn == null)
            {
                TPDR.DirectViewCond[] dvcArug = new TPDR.DirectViewCond[3];
                DataTable dt = null;

                dvcArug[0].sCondtion_ID = "FACTORY";
                dvcArug[0].sCondition_Value = MPGV.gsFactory;

                dvcArug[1].sCondtion_ID = "VIEW_ID";
                dvcArug[1].sCondition_Value = cvpm.ViewID;

                dvcArug[2].sCondtion_ID = "COL_NAME";
                dvcArug[2].sCondition_Value = cvpm.SelectedColumn;

                if (TPDR.GetDataOne("", ref dt, "COM0000-002", dvcArug, false, false) == false)
                {
                    if (dt != null)
                        dt.Dispose();
                ...
                return MPCF.ToInt(dt.Rows[0]["COL_SEQ"]) - 1;
            }
            else
            {
                for (int i = 0; i < cvpm.DisplayColumn.Length; i++)
            ...
            foreach (TRSNode node in outNodeList)
            {
                ...
                for (int i = 0; i < displayColumn.Length; i++)
                {
                    ...
                    if (i > headers.Length - 1)
                    {
                        ...
                        {
                            ugc = SOIGridAddColumn(targetGrid, MPCF.FindLanguage(displayColumn[i]), displayColumn[i]);
                        }
                        else
                        {
                            // Out Node의 컬럼값을 사용
                            ugc = SOIGridAddColumn(targetGrid, MPCF.FindLanguage(node.GetMember(displayColumn[i]).Name), displayColumn[i]);
                        }
                    }
                    // Header가 있는 경우
                    else
                    {
                        ugc = SOIGridAddColumn(targetGrid, MPCF.FindLanguage(headers[i]), displayColumn[i]);
                    }
            ...
            foreach (DataColumn dcData in dtData.Columns)
            {
                ...
                for (int i = 0; i < displayColumn.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (i > headers.Length - 1)
                        {
                            ugc = SOIGridAddColumn(targetGrid, MPCF.FindLanguage(dcData.ColumnName), displayColumn[i]);
                        }
                        // Header가 있는 경우
                        else
                        {
        ...
        private DataTable BindToSOIGrid(SOIGrid targetGrid, DataTable dtData)
        {
            string sTableKey = "codeViewPopup";
            int i = 0;
            ...
            targetGrid.SetDataSource(dtData);

            for (i = 0; i < targetGrid.DisplayLayout.Bands[0].Columns.Count; i++)
            {
                targetGrid.DisplayLayout.Bands[0].Columns[i].Header.Caption = MPCF.FindLanguage(targetGrid.DisplayLayout.Bands[0].Columns[i].Header.Caption.Replace('_', ' '));
            }
        ...
        {
            string sReturn = MPCF.FindLanguage("No data to display");
    ...
    public class CodeViewPopupModel_Mat
    {
        private BOIMatCodeView _control;
        public BOIMatCodeView Control
        {
            get { return _control; }
            set { _control = value; }
        }
        ...
        private TPDR.DirectViewCond[] _Argu;
        public TPDR.DirectViewCond[] Argu
        {
            get { return _Argu; }
            set { _Argu = value; }
        }
