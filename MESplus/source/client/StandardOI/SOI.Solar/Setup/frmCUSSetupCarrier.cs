...
namespace SOI.Solar
{
    ...
    public partial class frmCUSSetupCarrier : SOIBaseForm03
    {
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (CheckCondition("WIP_Adapt_Lot") == true)
                {
                    ...
                    // Set List
                    sOrderID = MPCF.Trim(cdvGroup.Text);
                    if (sOrderID == "") return;
        ...
        private bool ViewScreen(int selectedRowIndex)
        {
            ...
            try
            {
                ...
                if (spdCarrierList.Sheets[0].Rows.Count < 1)
                {
                    return false;
                }
                ...
                sPathZip = MPGV.gsFlexibleScreenLocalPath + spdCarrierList.Sheets[0].Cells[selectedRowIndex, (int)COLS_SCREEN_LIST.SCREEN_ID].Value.ToString() + ".gzip";
                ...
                in_node.ProcStep = '1';
                in_node.AddString("SCREEN_ID", spdCarrierList.Sheets[0].Cells[selectedRowIndex, (int)COLS_SCREEN_LIST.SCREEN_ID].Value.ToString());
                if (fi.Exists == false)
                {
                ...
                {
                    // File Update
                    UpdateScreenXML(spdCarrierList.Sheets[0].Cells[selectedRowIndex, (int)COLS_SCREEN_LIST.SCREEN_ID].Value.ToString(), out_node);
                }

                // 7. XML 파일 경로 설정
                sPathXML = MPGV.gsFlexibleScreenLocalPath + spdCarrierList.Sheets[0].Cells[selectedRowIndex, (int)COLS_SCREEN_LIST.SCREEN_ID].Value.ToString() + ".xml";
        ...
        private void ClearData(char ProcStep)
        {
            try
            {
                if (ProcStep == '1')
                {
                    MPCF.FieldClear(cdvGroup);
                    MPCF.ClearList(spdCarrierList);
                }
        ...
        private bool ViewSourceOrderList()
        {
            ...
            try
            {
                ...
                in_node.AddString("ORDER_ID", MPCF.Trim(cdvGroup.Text));
                ...
                {
                    spdCarrierList.ActiveSheet.RowCount++;
                    spdCarrierList.ActiveSheet.Cells[i, (int)SOURCE_LIST.ORDER_ID].Value = order_list[i].GetString("ORDER_ID");
                    spdCarrierList.ActiveSheet.Cells[i, (int)SOURCE_LIST.ORDER_DESC].Tag = order_list[i].GetString("ORDER_DESC");
                    //spdSourceOrderList.ActiveSheet.Cells[i, (int)SOURCE_LIST.MAT_ID].Text = order_list[i].GetString("MAT_ID");
                }

                MPCF.FitColumnHeader(spdCarrierList);
        ...
        private void cdvGroup_Click(object sender, EventArgs e)
        {
            int i_cond_cnt;
            string sViewID;
            ...
            {
                i_cond_cnt = 1;
                sViewID = "VIEW_CARRIER_GROUP_LIST";
                ...
                string[] header = new string[] { "Carrier Group", "Description" };

                // CodeView Display Column Setup
                string[] display = new string[] { "CRR_GROUP", "CRR_GRP_DESC" };

                // Show by RPTServer
                cdvGroup.Text = cdvGroup.Show(cdvGroup, "Carrier Group", sViewID, dvcArgu, display, header, "CRR_GROUP", -1);

                if (cdvGroup.returnDatas != null && cdvGroup.returnDatas.Count > 0)
                {
                    cdvGroup.Tag = cdvGroup.returnDatas[0];
                    cdvGroup.Text = cdvGroup.returnDatas[0];
                }
            }
            catch (Exception ex)
            {
        ...
        private bool ViewCarrierList()
        {
            ...
            MPCF.ClearList(spdCarrierList);
            spdCarrierList.ActiveSheet.RowCount = 0;

            try
            {
                i_cond_cnt = 1;
                sViewID = "VIEW_CARRIER_LIST";
                ...
                if (TPDR.DirectViewOne(spdCarrierList, sViewID, ref dt, false, false, dvcArgu, true) == false)
                {
        ...
        private void spdSourceOrderList_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            ...
            {
                string sOrderID;
                int iRow = spdCarrierList.ActiveSheet.ActiveRowIndex;
                if (iRow < 0) return;

                sOrderID = MPCF.Trim(spdCarrierList.ActiveSheet.Cells[iRow, 0].Value);
                if (sOrderID == "") return;

                //if (ViewLotList(sOrderID) == false) return;

                cdvGroup.Text = sOrderID;
            }
            catch (Exception ex)
            {
                MPCF.ShowMessage(ex.Message, MSG_LEVEL.ERROR, false);
