...
namespace SOI.Solar
{
    public partial class frmCUSTranTempMatShipManagement : SOIBaseForm03
    {
        #region [Property]
        ...
        public frmCUSTranTempMatShipManagement()
        {
            InitializeComponent();
        }
            ...
            try
            {
                switch (FuncName)
                {
                    ...
                    case CSGC.MP_CHECK_UPDATE:
                        ...
                        if (!CheckSaveExist()) //출하처리 이전 저장여부 Pallet 로 조회
                        {
        ...
        private bool ViewShipmentList()
        {
            ...
            try
            {
                ...
                //    sViewID = "VIEW_SHIPMENT_TEMP_LIST_ALL";
                //else
                //    sViewID = "VIEW_SHIPMENT_TEMP_LIST";
                sViewID = "VIEW_SHIPMENT_TEMP_LIST_ALL_02";
        ...
        private bool ViewShipmentMaterialList(VALIDATION_CASE valCase, bool deleteFlag = true)
        {
            ...
            try
            {
                sViewID = "VIEW_TEMP_SHIP_MAT_LIST_02";
                i_cond_cnt = 7;
        ...
        private bool ViewTargetMatList()
        {
            ...
            {
                if (chkFinishFlag.Checked)
                    sViewID = "VIEW_TARGET_MAT_TEMP_LIST_ALL";
                else
                    sViewID = "VIEW_TARGET_MAT_TEMP_LIST";
        ...
        private bool SavePalletLotList()
        {
            ...
            try
            {
                ...
                in_node.AddString("CMF_1", "Y"); // 긴급 플래그
            ...
            try
            {
                ...
                for (int i = 0; i < spdTargetMatList.ActiveSheet.RowCount; i++)
                {
                    if (Convert.ToBoolean(spdTargetMatList.ActiveSheet.Cells[i, (int)TARGET_MAT_LIST.CHECKBOX].Value) == true)
                    {
                        ...
                        // 20171212-긴급 일 때는 ERP I/F 출하 T_PD_PODD 테이블을 업데이트 하지 않는다. 향후에 '긴급 출고 의뢰 지정' 하면 넣어야 함
                        list_item.AddChar("EMERGENCY_SHIPMENT", 'Y');
        ...
        private bool GetTransportationInfo()
        {
            ...
            try
            {
                ...
                ArrDVC[1].sCondtion_ID = "$ORDER_DATE";
                ArrDVC[1].sCondition_Value = MPCF.Trim(saveCarryValues.ORDER_DATE);

                ArrDVC[2].sCondtion_ID = "$CONTRACT_NO";
                ArrDVC[2].sCondition_Value = MPCF.Trim(saveCarryValues.CONTRACT_NO);
        ...
        private bool GetProductDetailInfo()
        {
            ...
            try
            {
                //sViewID = "VIEW_PRODUCT_PRINT_INFO";
        ...
        private void frmCUSTranTempMatShipManagement_Load(object sender, EventArgs e)
        {
            initCtrl();
        }

        private void frmCUSTranTempMatShipManagement_Shown(object sender, EventArgs e)
        {
            if (bIsShown == false)
            {
                bIsShown = true;
        ...
        private void txtPalletID_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == ENTER)
            {
                txtPalletID.Text = MPCF.ToUpper(txtPalletID.Text); //일괄 대문자
