...
namespace SOI.WIP
{
    ...
    public partial class frmWIPRepairLot : SOIBaseForm02
    {
        #region Property
        ...
        /// <param name="e"></param>
        private void frmWIPRepairLot_Load(object sender, EventArgs e)
        {
            // Init
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmWIPRepairLot_Shown(object sender, EventArgs e)
        {
            // 최초 1회 실행
            if (bIsShown == false)
            {
        ...
        /// <summary>
        /// Repair Code CodeView
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void cdvRepairCode_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                // In Node Setup
                ...
                in_node.ProcStep = '1';
                in_node.AddString("TABLE_NAME", MPCF.Trim(MPGC.MP_WIP_REPAIR_CODE));

                // Display and Header Text Setup
                string[] display = new string[] { "KEY_1", "DATA_1" };
                string[] header = new string[] { "Repair Code", "Repair Desc" };

                // Show CodeView and Get Return
                cdvRepairCode.Text = cdvRepairCode.Show(cdvRepairCode, "BAS", "View Repair Code", "BAS_View_Data_List", in_node, "DATA_LIST", display, header, "Repair Code");
            }
            catch (Exception ex)
            {
                MPCF.ShowMessage(ex.Message, MSG_LEVEL.ERROR, false);
        ...
        /// <summary>
        /// Repair Oper CodeView
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void cdvRepairOper_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                // Display and Header Text Setup
                string[] display = new string[] { "REP_OPER", "REP_OPER_DESC" };
                string[] header = new string[] { "Rep Oper", "Rep Oper Desc" };

                // Show CodeView and Get Return
                cdvRepairOper.Text = cdvRepairOper.Show(cdvRepairOper, "WIP", "View Repair Operation List", "WIP_View_Repair_Oper_List", in_node, "LIST", display, header, "Rep Oper");
            }
            catch (Exception ex)
            {
                MPCF.ShowMessage(ex.Message, MSG_LEVEL.ERROR, false);
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MPCF.CheckValue(cdvRepairCode, false) == false)
                {
                    return;
                }
                if (MPCF.CheckValue(cdvRepairOper, false) == false)
                {
                    return;
                }
        ...
        /// </summary>
        /// <returns></returns>
        private bool RepairLot()
        {
            TRSNode in_node = new TRSNode("REPAIR_LOT_IN");
            TRSNode out_node = new TRSNode("REPAIR_LOT_OUT");
            ...
            try
            {
                ...
                in_node.AddString("REPAIR_CODE_1", cdvRepairCode.Text);
                in_node.AddString("TO_OPER", cdvRepairOper.Text);
                in_node.AddString("RET_OPER", LOT.GetString("OPER"));
                in_node.AddString("RES_ID", LOT.GetString("START_RES_ID"));
                ...
                if (MPCF.CallService("WIP", "WIP_Repair_Lot", in_node, ref out_node) == false)
                {
                    return false;
                }
