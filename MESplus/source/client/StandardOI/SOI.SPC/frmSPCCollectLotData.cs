...
namespace SOI.SPC
{
    ...
    public partial class frmSPCCollectLotData : SOIBaseForm02
    {
        #region Property
        ...
        private void cdvChartID_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                in_node.AddString("RES_ID", LOT.GetString("RES_ID"));                
                ...
                cdvChartID.Text = cdvChartID.Show(cdvChartID, "View Chart List", "SPC", "SPC_View_Chart_List", in_node, "CHART_LIST", display, header, "Chart ID");

                // Field Clear
                MPCF.FieldClear(this, txtLotID, cdvChartID, spdChartInfo);
                ClearChartInfo();

                // Validation
                if (string.IsNullOrEmpty(cdvChartID.Text) == true)
                {
                    return;
                }
        ...
        private void btnControlChart_Click(object sender, EventArgs e)
        {
            ...
            {
                // Validation
                if (MPCF.CheckValue(cdvChartID, false) == false)
                {
                    return;
                }
        ...
        private bool ViewLot(string sLotId)
        {
            ...
            try
            {
                ...
                // Field Clear
                MPCF.FieldClear(this, txtLotID, spdChartInfo);
                ClearChartInfo();
        ...
        /// <param name="iValueCol1"></param>
        /// <returns></returns>
        public static bool SetValuePrompt(FarPoint.Win.Spread.FpSpread spddata, string sChartID, int iSampleSize, int iValueCol1)
        {
        ...
        private void SetCollectData()
        {
            try
            {
                ChartID = MPCF.Trim(cdvChartID.Text);
                LotResFlag = 'L';
                LotID = MPCF.Trim(txtLotID.Text);
                MatID = MPCF.Trim(LOT.GetString("MAT_ID"));
                MatVer = LOT.GetInt("MAT_VER");
                Flow = MPCF.Trim(LOT.GetString("FLOW"));
                FlowSeq = LOT.GetInt("FLOW_SEQ_NUM");
                Oper = MPCF.Trim(LOT.GetString("OPER"));
                CharID = MPCF.Trim(cdvCharID.Text);
                UserID = MPCF.Trim(LOT.GetString("CREATE_USER"));
            }
            catch (Exception ex)
            {
                MPCF.ShowMessage(ex.Message, MSG_LEVEL.ERROR, false);
        ...
        public bool CollectEDCData(char c_step)
        {
            ...
            {                
                ...
                {
                    DialogResult result;
                    result = MPCF.ShowMsgBox(out_node.GetString(TRSDefine.OUT_MSG), MessageBoxButtons.YesNo);
