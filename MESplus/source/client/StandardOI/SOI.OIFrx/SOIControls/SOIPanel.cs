...
namespace SOI.OIFrx.SOIControls
{
    public partial class SOIPanel : Panel
    {
        ...
        private bool _setAutoScrollPanel = false;
        public bool _SetAutoScrollPanel
        {
            get { return _setAutoScrollPanel; }
            set
            {
                _setAutoScrollPanel = value;
                SetAutoScrollPanel(value);
            }
        }
        ...
        public void SetOITheme()
        {
            if (_UseOITheme == true)
            {
                ...
                {
                    this.BackColor = MPGV.gTheme.PanelBackground;
                }
                // TabControl의 Border로 사용되는 경우
                else if (_UseStyle == SOIPanelStyle.TabControlBorderStyle)
                {
                    this.BackColor = MPGV.gTheme.TabControlBorder;
                }
            }
        }

        private void SetAutoScrollPanel(bool set)
        {
            if (set == true)
            {
                this.AutoScrollMinSize = new Size(0, 200);
