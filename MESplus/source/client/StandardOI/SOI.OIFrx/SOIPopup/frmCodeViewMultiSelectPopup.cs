...
namespace SOI.OIFrx.SOIPopup
{
    ...
    public partial class frmCodeViewMultiSelectPopup : frmPopupBase
    {
        ...
        public bool InitCodeViewPopup(SOICodeViewMultiSelect control, string popupTitle, string moduleName, string serviceName, TRSNode inNode,
            string listName, string[] displayColumn, string[] header, string[] visibleFalse, string selectedColumn, int pageSize, List<string> returnValues, TRSNode returnData)
        {
            // 생성
            if (cvpm == null)
            {
        ...
        private bool ViewService(bool initialize)
        {
            try
            {
                ...
                // Data Bind
                dtOriginalData = BindToSOIGrid(grdAllData, out_node.GetList(cvpm.ListName), cvpm.DisplayColumn, cvpm.Header, cvpm.VisibleFalse, false);

                // Initialize를 위한 초기화 구문
                if (initialize == true)
                {
                    grdSelectedData.ClearDataSource();
                    dtSelectedData = BindToSOIGrid(grdSelectedData, out_node.GetList(cvpm.ListName), cvpm.DisplayColumn, cvpm.Header, cvpm.VisibleFalse, true);
                }                
        ...
        private void FindData()
        {
            try
            {
                ...
                if (dtOriginalData != null)
                {
                    ...
                    // Bind
                    BindToSOIGrid(grdAllData, dtOriginalData.DefaultView.ToTable(), cvpm.DisplayColumn, cvpm.Header, cvpm.VisibleFalse);
                }
#endif
#if _Http
                // Pagenation인 경우
        ...
        /// <returns></returns>
        private DataTable BindToSOIGrid(SOIGrid targetGrid, List<TRSNode> outNodeList, string[] displayColumn, string[] headers, string[] visibleFalseColumn, bool selected)
        {
