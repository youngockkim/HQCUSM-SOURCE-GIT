...
namespace SOI.OIFrx.SOIPopup
{
    public partial class frmCodeViewPopup : frmPopupBase
    {
        ...
        private void gridCodeViewPopup_ClickCell(object sender, ClickCellEventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(cvpm.Control._ValueColumn) == false)
                {
                    int iValueIndex = GetIndexDisplayColumn(cvpm.Control._ValueColumn);
                    cvpm.Control.Value = gridCodeViewPopup.Selected.Rows[0].Cells[iValueIndex].Text;
                }
                else
                {
                    cvpm.Control.Value = this.ReturnValue;
                }
        ...
        private void btnFieldsReset_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(cvpm.Control._ValueColumn) == false)
            {
                cvpm.Control.Value = string.Empty;
            }
        ...
        /// <returns></returns>
        public bool InitCodeViewPopup(SOICodeView control, string popupTitle, string moduleName, string serviceName, TRSNode inNode,
            string listName, string[] displayColumn, string[] header, string[] visibleFalse, string selectedColumn, int pageSize)
        {
            ...
            cvpm.Header = header;
            cvpm.VisibleFalse = visibleFalse;
        ...
        private bool ViewService()
        {
            try
            {
                ...
                // Data Bind
                dtOriginalData = BindToSOIGrid(gridCodeViewPopup, out_node.GetList(cvpm.ListName), cvpm.DisplayColumn, cvpm.Header, cvpm.VisibleFalse);
        ...
        private void InitFindComboBox()
        {
            ...
            {
                // Visible False Column은 제외
                if (cvpm.VisibleFalse != null && cvpm.VisibleFalse.Length > 0)
                {
                    bool bFound = false;
                    foreach (string col in cvpm.VisibleFalse)
                    {
                        if (col == cvpm.DisplayColumn[i])
                        {
                            bFound = true;
                            break;
                        }
        ...
        private void FindData()
        {
            try
            {
                ...
                if (dtOriginalData != null)
                {
                    ...
                    // Bind
                    BindToSOIGrid(gridCodeViewPopup, dtOriginalData.DefaultView.ToTable(), cvpm.DisplayColumn, cvpm.Header, cvpm.VisibleFalse);
                }
#endif
#if _Http
                // Pagenation인 경우
        ...
        /// <returns></returns>
        private DataTable BindToSOIGrid(SOIGrid targetGrid, List<TRSNode> outNodeList, string[] displayColumn, string[] headers, string[] visibleFalseColumn)
        {
            ...
            foreach (TRSNode node in outNodeList)
            {
                ...
                {
                    // Column 생성
                    // Header에 정의하지 않은 경우, Node의 Member이름을 사용
                    ...
                    // Column Visible 설정
                    if (visibleFalseColumn != null && visibleFalseColumn.Length > 0)
                    {
                        foreach (string col in visibleFalseColumn)
                        {
                            if (col == displayColumn[i])
                            {
                                ugc.Hidden = true;
                                break;
                            }
        ...
        /// <param name="headers"></param>
        /// <returns></returns>
        private DataTable BindToSOIGrid(SOIGrid targetGrid, DataTable dtData, string[] displayColumn, string[] headers, string[] visibleFalseColumn)
        {
            string sTableKey = "codeViewPopup";
            DataColumn dc;
            DataRow dr;
            ...
            // dtData: 검색조건으로 Filtering 된 이후의 데이터
    ...
    public class CodeViewPopupModel
    {
        ...
        private string[] _visibleFalse;
        public string[] VisibleFalse
        {
            get { return _visibleFalse; }
            set { _visibleFalse = value; }
        }
