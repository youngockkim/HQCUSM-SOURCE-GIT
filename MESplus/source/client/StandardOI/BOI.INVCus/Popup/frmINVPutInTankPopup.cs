...
namespace BOI.INVCus
{
    ...
    public partial class frmINVPutInTankPopup : frmPopupBase
    {
        #region Property
        ...
        private void spdCarResList_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            try
            {
                if (MPCF.ToChar(spdCarResList.Sheets[0].Cells[e.Row, (int)ColResourceList.CONFIRM_FLAG].Value) == 'Y')
                {
                    ...
                    spdCarResList.Sheets[0].Cells[e.Row, (int)ColResourceList.WEIGHT_TANK].Locked = true;
                    spdCarResList.Sheets[0].SetValue(e.Row, (int)ColResourceList.SELECT, false);
                    //MPCF.ShowMsgBox(MPCF.GetMessage(433), MessageBoxButtons.OK, MSG_LEVEL.ERROR, "", true);
                    return;
                }
        ...
        private bool SetAuto()
        {
            ...
            try
            {
                ...
                //    if (spdCarResList.Sheets[0].GetValue(i, (int)ColResourceList.SELECT) != null)
                //    {
                        
                //        if ((bool)spdCarResList.Sheets[0].GetValue(i, (int)ColResourceList.SELECT) == true)
                //        {
                //            iCheckCount++;
                //        }
                //        if ((string)spdCarResList.Sheets[0].GetValue(i, (int)ColResourceList.CONFIRM_FLAG) == "Y")
                //        {
                //            return false;
                //        }
                //    }
                ...
                //    if (iCheckCount > 1)
                //    { //선택항목이 2개 이상일 경우 에러처리
                //        MPCF.ShowMsgBox(MPCF.GetMessage(452), MessageBoxButtons.OK, MSG_LEVEL.ERROR, "", true);
                //        return false;
                //    }
                //}
                ...
                //if (iCheckCount == 0)
                //{
                //    MPCF.ShowMsgBox(MPCF.GetMessage(453), MessageBoxButtons.OK, MSG_LEVEL.ERROR, "", true);
                //    return false;
                //}
                ...
                //        if ((bool)spdCarResList.Sheets[0].GetValue(i, (int)ColResourceList.SELECT) == true)
                //        {
                //            spdCarResList.Sheets[0].SetValue(i, (int)ColResourceList.WEIGHT_TANK, numRemainQty.Value);
                //        }
                //    }
                ...
                if ((string)spdCarResList.Sheets[0].GetValue(iActiveRow, (int)ColResourceList.CONFIRM_FLAG) == "Y") //확정된 항목일때 예외처리.
                {
                    return false;
                }
        ...
        private bool UpdateWeightRawmilk()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < spdCarResList.Sheets[0].RowCount; i++)
                {
                    if (MPCF.ToDbl(spdCarResList.Sheets[0].Cells[i, (int)ColResourceList.WEIGHT_TANK].Value) > 0 ||
                        (MPCF.ToDbl(spdCarResList.Sheets[0].Cells[i, (int)ColResourceList.WEIGHT_TANK].Value) == 0 &&
                        MPCF.ToChar(spdCarResList.Sheets[0].Cells[i, (int)ColResourceList.IUD_FALG].Value) == 'U'))
                    {
                        ...
                        {
                            tank_list.ProcStep = MPCF.ToChar(spdCarResList.Sheets[0].Cells[i, (int)ColResourceList.IUD_FALG].Value);
                        }
                        tank_list.AddString("INV_REQ_NO", MPCF.Trim(invReqNo));
                        tank_list.AddInt("INV_REQ_SEQ", MPCF.ToInt(InvReqSeq));
                        tank_list.AddString("RES_ID", MPCF.Trim(spdCarResList.Sheets[0].Cells[i, (int)ColResourceList.TANK_ID].Value));
                        ...
                        tank_list.AddDouble("WEIGHT_REAL", MPCF.ToDbl(spdCarResList.Sheets[0].Cells[i, (int)ColResourceList.WEIGHT_TANK].Value));
                ...
                if (MPCF.CallService("BINV", "BINV_Update_Rawmilk_Tank", in_node, ref out_node) == false)
                {
                    return false;
                }
        ...
        {
            spdCarResList.Sheets[0].Columns[(int)ColResourceList.SELECT].Visible = false;
            spdCarResList.Sheets[0].Columns[(int)ColResourceList.IUD_FALG].Visible = false;
            spdCarResList.Sheets[0].Columns[(int)ColResourceList.DEFAULT_TANK].Visible = false;
            spdCarResList.Sheets[0].Columns[(int)ColResourceList.CONFIRM_FLAG].Visible = false;
            spdCarResList.Sheets[0].Columns[(int)ColResourceList.TANK_ID].Visible = false;
            spdCarResList.Sheets[0].Columns[(int)ColResourceList.OPER].Visible = false;
