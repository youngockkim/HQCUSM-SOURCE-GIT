...
namespace BOI.INVCus
{
    ...
    public partial class frmINVBulkLossInventoryLot : BOIBaseForm02
    {
        #region Property
        ...
        public frmINVBulkLossInventoryLot()
        {
            InitializeComponent();
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                spdInvLot.Sheets[0].Cells[spdInvLot.Sheets[0].RowCount - 1, (int)MAT_LOT_COL.UNIT].Value = txtMatUnit.Text;
        ...
        private void txtMatLotID_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtInvLotID.Text != "" && CheckCondition("VIEW_LOT") == true)
                {
                    if (View_Lot(txtInvLotID.Text) == false)
                    {
                        ClearData('1');
        ...
        private void spdLotList_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            try
            {
                txtInvLotID.Text = spdLotList.Sheets[0].Cells[e.Row, 0].Value.ToString();
            }
            catch (Exception ex)
            {
                MPCF.ShowMessage(ex.Message, MSG_LEVEL.ERROR, false);
        ...
        private void View_Lot_List()
        {
            try
            {
                ...
                dvcArgu[1].sCondition_Value = cdvStoreID.Tag.ToString().Trim();

                dvcArgu[2].sCondtion_ID = "MAT_ID";
                dvcArgu[2].sCondition_Value = cdvMatID.Text.Trim();

                if (TPDR.GetDataOne(s_column, ref dt, "OINV2206-001", dvcArgu, false, false) == false)
                {
                    if (dt != null)
                        dt.Dispose();
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdLotList.Sheets[0].Cells[i, 0].Value = dt.Rows[i]["LOT_ID"].ToString();
                }
