...
namespace BOI.INVCus
{
    ...
    public partial class frmINVShipInventoryLot : BOIBaseForm02
    {
        #region Property
        ...
        public frmINVShipInventoryLot()
        {
            InitializeComponent();
        }

        public frmINVShipInventoryLot(string args)
        {
            InitializeComponent();
        ...
        private void frmTempBOIBaseForm02_Load(object sender, EventArgs e)
        {
            ...
            dtpToDate.SetValueAsDateTime(DateTime.Now);
        ...
        private void frmTempBOIBaseForm02_Shown(object sender, EventArgs e)
        {
            ...
            if (bIsShown == false)
            {
                ...
                {
                    View_Req_Mst(_sparaReqDate, _sparaReqDate, _sparaInvReqNo);
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (View_Req_Mst(dtpDate.GetValueAsString(8), dtpToDate.GetValueAsString(8)) == false)
                {
                    //Nothing
                }
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            try
            {
                if (CheckCondition("SHIP_LOT") == true)
                {
                    if (Ship_Lot('1') == true)
                    {
                        btnView.PerformClick();
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                file_path = "";
                frmInvViewInvMove frm = new frmInvViewInvMove(MPCF.Trim(_sInvReqNo));
                selectedMenuInfo = new MenuInfoTag();
                selectedMenuInfo.s_func_desc = "View Inv Return";
        ...
        private void InvisibleColumn()
        {
            try
            {
                ...
                spdReqMstList.Sheets[0].Columns[(int)REQ_MST_COL.FROM_FACTORY_CODE].Visible = false;
                spdReqMstList.Sheets[0].Columns[(int)REQ_MST_COL.FROM_STORE_CODE].Visible = false;
                spdReqMstList.Sheets[0].Columns[(int)REQ_MST_COL.TO_FACTORY_CODE].Visible = false;
        ...
        private bool View_Req_Mst(string reqFromDate, string reqToDate)
        {
            return View_Req_Mst(reqFromDate, reqToDate, "");
        }
        ...
        /// <returns></returns>
        private bool View_Req_Mst(string reqFromDate, string reqToDate, string invReqNo)
        {
            try
            {
                MPCF.ClearList(spdReqMstList);
                ...
                dvcArgu[1].sCondition_Value = BIGC.B_REQ_TYPE_SHIP;

                dvcArgu[2].sCondtion_ID = "REQ_FROM_DATE";
                dvcArgu[2].sCondition_Value = reqFromDate;

                dvcArgu[3].sCondtion_ID = "REQ_TO_DATE";
                dvcArgu[3].sCondition_Value = reqToDate;

                dvcArgu[4].sCondtion_ID = "INV_REQ_NO";
                dvcArgu[4].sCondition_Value = invReqNo;

                if (TPDR.GetDataOne(s_column, ref dt, "BINV1002-001", dvcArgu, false, false) == false)
                {
                    if (dt != null)
                        dt.Dispose();
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    spdReqMstList.Sheets[0].Cells[spdReqMstList.Sheets[0].RowCount - 1, (int)REQ_MST_COL.FROM_FACTORY_DESC].Value = dt.Rows[i]["FROM_FACTORY_DESC"].ToString();
                    spdReqMstList.Sheets[0].Cells[spdReqMstList.Sheets[0].RowCount - 1, (int)REQ_MST_COL.FROM_FACTORY_CODE].Value = dt.Rows[i]["FROM_FACTORY_CODE"].ToString();
                    ...
                    spdReqMstList.Sheets[0].Cells[spdReqMstList.Sheets[0].RowCount - 1, (int)REQ_MST_COL.TO_FACTORY_DESC].Value = dt.Rows[i]["TO_FACTORY_DESC"].ToString();
                    spdReqMstList.Sheets[0].Cells[spdReqMstList.Sheets[0].RowCount - 1, (int)REQ_MST_COL.TO_FACTORY_CODE].Value = dt.Rows[i]["TO_FACTORY_CODE"].ToString();
                    ...
                    spdReqMstList.Sheets[0].Cells[spdReqMstList.Sheets[0].RowCount - 1, (int)REQ_MST_COL.SHIPMENT_NO].Value = dt.Rows[i]["SHIPMENT_NO"].ToString();
        ...
        private bool View_Req_Detail(string sInvReqNo, char cReqStatus)
        {
            try
            {
                ...
                if (TPDR.GetDataOne(s_column, ref dt, "BINV1002-002", dvcArgu, false, false) == false)
                {
                    if (dt != null)
                        dt.Dispose();
        ...
        private bool View_Picking_Lot(string sMatId, int iInvReqSeq)
        {
            try
            {
                ...
                if (TPDR.GetDataOne(s_column, ref dt, "BINV1002-003", dvcArgu, false, false) == false)
                {
                    if (dt != null)
                        dt.Dispose();
        ...
        //
        private bool Ship_Lot(char ProcStep)
        {
            TRSNode in_node = new TRSNode("SHIP_LOT_IN");
            TRSNode out_node = new TRSNode("SHIP_LOT_OUT");
            string sFromFactory = string.Empty;
            string sToFactory = string.Empty;
            ...
            try
            {
                ...
                sFromFactory = spdReqMstList.Sheets[0].Cells[spdReqMstList.Sheets[0].ActiveRow.Index, (int)REQ_MST_COL.FROM_FACTORY_CODE].Value.ToString();
                sToFactory = spdReqMstList.Sheets[0].Cells[spdReqMstList.Sheets[0].ActiveRow.Index, (int)REQ_MST_COL.TO_FACTORY_CODE].Value.ToString();

                if (!sFromFactory.Equals(sToFactory))
                {
                    //INV_CMF_1 계정그룹
                    in_node.AddString("LOT_CMF_1", BIGC.B_INV_ACC_GRP_ISS_SALES);

                    //INV_CMF_2 계정상세유형 
                    in_node.AddString("LOT_CMF_2", BIGC.B_INV_ACC_DETAIL_ISS_FACTORY);

                    //사유그룹
                    in_node.AddString("LOT_CMF_7", ""); //없으면 서버에서 처리

                    //사유
                    in_node.AddString("LOT_CMF_8", MPCF.Trim(_sReason)); //사유코드

                    if (MPCF.CallService("BINV", "BINV_Tran_Ship_Inventory_Lot", in_node, ref out_node) == false)
                    {
                        return false;
                    }
