...
namespace BOI.INVCus
{
    ...
    public partial class frmINVMoveInventoryLot : BOIBaseForm02
    {
        #region Property
        ...
        private void cdvFromStoreID_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                // Show
                cdvFromStoreID.Text = cdvFromStoreID.Show(cdvFromStoreID, "Store ID", "BWIP", "BWIP_View_Operation_List", in_node, "OPERATION_LIST", display, header, "OPER_SHORT_DESC");

                if (MPCF.Trim(cdvFromStoreID.Text) != "")
                {
                    if (cdvFromStoreID.returnDatas.Count > 0)
                    {
                        cdvFromStoreID.Tag = cdvFromStoreID.returnDatas[0];
                        cdvFromStoreID.Text = cdvFromStoreID.returnDatas[1];

                        if (cdvFromStoreID.Tag.ToString() == cdvToStoreID.Tag.ToString())
                        {
                            cdvToStoreID.Text = "";
                            cdvToStoreID.Tag = "";
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                spdInvLot.Sheets[0].Cells[spdInvLot.Sheets[0].RowCount - 1, (int)INV_LOT_COL.FROM_STORE_CODE].Value = cdvFromStoreID.Tag;
            ...
            try
            {
                ...
                for (i = 0; i < spdInvLot.Sheets[0].RowCount; i++)
                {
                    if (spdInvLot.Sheets[0].Cells[i, (int)INV_LOT_COL.CHECK_SELECT].Value != null && (bool)spdInvLot.Sheets[0].Cells[i, (int)INV_LOT_COL.CHECK_SELECT].Value == true)
                    {
                        ...
                        list_item.AddString("LOT_CMF_8", MPCF.Trim(cdvReason.Tag)); //LOT_CMF_8 사유
                    }
                }
        ...
        private bool View_Lot(string sLotID)
        {
            ...
            try
            {
                ...
                cdvFromStoreID.Text = out_node.GetString("OPER_DESC");
                cdvFromStoreID.Tag = out_node.GetString("OPER");
        ...
        private void cdvReason_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                in_node.AddString("DATA_2", BIGC.B_INV_RSN_GRP_RG3);
                ...
                // Show
                cdvReason.Text = cdvReason.Show(cdvReason, "Category 1", "BCOM", "BCOM_View_Gcm_Data_List", in_node, "GCM_DATA_LIST", display, header, "DATA_1");
