...
namespace BOI.INVCus
{
    ...
    public partial class frmINVDivideInventoryLot : BOIBaseForm02
    {
        #region Property
        ...
        public frmINVDivideInventoryLot()
        {
            InitializeComponent();
        }
        ...
        private void ClearData(int i)
        {
            if (i == 1)
            {
                ...
                txtDivideQty.Value = txtDivideQty.NullText;
                spdQty.ActiveSheet.ColumnHeader.Cells[1, 1].Text = " ";
                spdQty.ActiveSheet.ColumnHeader.Cells[1, 2].Text = " ";

                spdQty.ActiveSheet.Cells[0, 0].Value = 0.00;
                spdQty.ActiveSheet.Cells[0, 1].Value = 0.00;
                spdQty.ActiveSheet.Cells[0, 2].Value = 0.00;
            }
            return;
        }
            ...
            try
            {
                switch (FuncName)
                {
                    ...
                    case "SPLIT_LOT":
                        ...
                        //수량 체크
                        if (MPCF.ToDbl(txtDivideQty.Value) <= 0.00)
                        {
                        ...
                        if (MPCF.ToDbl(txtDivideQty.Value) > MPCF.ToDbl(txtInvQty.Value))
                        {
                        ...
                        if (MPCF.ToDbl(txtDivideQty.Value) == MPCF.ToDbl(txtInvQty.Value))
                        {
                            if (MPCF.ShowMsgBox(MPCF.GetMessage(191), MessageBoxButtons.YesNo, MSG_LEVEL.NONE, "") == System.Windows.Forms.DialogResult.No)
                            {
                                return false;
                            }
                        }
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            try
            {
                if (CheckCondition("SPLIT_LOT") == true)
                {
                    if (Split_Inv_Lot('1') == true)
                    {
        ...
        //
        private bool Split_Inv_Lot(char ProcStep)
        {
            ...
            try
            {
                ...
                in_node.AddString("MAT_ID", MPCF.Trim(txtInvMatID.Text));
                in_node.AddDouble("SPLIT_QTY", MPCF.ToDbl(txtDivideQty.Value));

                if (MPCF.CallService("BINV", "BINV_Tran_Split_Inv_Lot", in_node, ref out_node) == false)
                {
                    txtInvLotID.Focus();
                    txtInvLotID.SelectAll();
                    return false;
                ...
                spdQty.ActiveSheet.ColumnHeader.Cells[1, 1].Text = MPCF.Trim(out_node.GetString("MOTHER_LOT_ID"));
                spdQty.ActiveSheet.ColumnHeader.Cells[1, 2].Text = MPCF.Trim(out_node.GetString("CHILD_LOT_ID"));
                
                spdQty.ActiveSheet.Cells[0, 1].Value = out_node.GetDouble("REMAIN_QTY");
                spdQty.ActiveSheet.Cells[0, 2].Value = out_node.GetDouble("SPLIT_QTY");
        ...
        private void btnLabelPrint_Click(object sender, EventArgs e)
        {
            ...
            //모랏
            if (Tran_Print_Label(MPCF.Trim(spdQty.ActiveSheet.ColumnHeader.Cells[1, 1].Text)) == false)
            {
                return;
            }
            //자랏
            if (Tran_Print_Label(MPCF.Trim(spdQty.ActiveSheet.ColumnHeader.Cells[1, 2].Text)) == false)
            {
                return;
            }
