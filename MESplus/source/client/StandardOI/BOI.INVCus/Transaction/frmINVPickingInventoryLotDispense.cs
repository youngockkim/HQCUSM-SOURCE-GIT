...
namespace BOI.INVCus
{
    ...
    public partial class frmINVPickingInventoryLotDispense : BOIBaseForm02
    {
        #region Enum
        ...
        private cinvReqMstDispense reqMst = null;
        //private List<cinvReqDtlDispense> lisReqDtl = null;
        //private List<cinvReqLotDispense> lisReqLot = null;
        ...
        public frmINVPickingInventoryLotDispense()
        {
            InitializeComponent();
        }
        ...
        private void txtLotId_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                cinvReqDtlDispense reqDtl = null;
                cinvReqLotDispense reqLot = null;
                ...
                if (e.KeyValue == (char)13)
                {
                    ...
                    if (reqMst == null)
                    {
                        reqMst = new cinvReqMstDispense("CINVREQMST_IN");
                    ...
                    reqDtl = new cinvReqDtlDispense(smatId);
                    ...
                    if (reqMst.lisReqDtl == null)
                    {
                        ...
                        reqLot = new cinvReqLotDispense(slotId);
                    ...
                    {
                        int irow = 0;
                        foreach (cinvReqDtlDispense reqDtlValue in reqMst.lisReqDtl)
                        {                            
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (spdReqLotMove.Sheets[0].RowCount > 0)
                {
                    ...
                    foreach (cinvReqDtlDispense reqDtl in reqMst.lisReqDtl)
                    {
                        ilotRow = 0;
                        foreach (cinvReqLotDispense reqLot in reqDtl.lisReqLot)
                        {
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MPCF.CallService("BINV", "BINV_Tran_Picking_Material_For_Dispense", reqMst, ref picking_out_node) == false)
                {
                    return;
                }
                ...
                //BICF.OpenMenu(BIGC.MP_FUNC_NAME_CINV2215, args);
                BICF.OpenMenu(BIGC.MP_FUNC_NAME_OINV2212, args);
            }
            catch (Exception ex)
            {
                MPCF.ShowMessage(ex.Message, MSG_LEVEL.ERROR, false);
        ...
        private void frmINVPickingInventoryLotDispense_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void SetSpread(cinvReqMstDispense reqMst)
        {
            int row = 0;
            if (reqMst == null)
            {
                ...
                spdReqMstMove.Sheets[0].SetTag(row, (int)ColReqMst.TO_STORE, MPCF.Trim(cdvToStore.Tag));
                spdReqMstMove.Sheets[0].SetValue(row, (int)ColReqMst.TO_STORE, MPCF.Trim(cdvToStore.Text));
    ...
    public class cinvReqMstDispense :TRSNode
    {                
        ...
        public List<cinvReqDtlDispense> lisReqDtl { get; set; }
        
        public cinvReqMstDispense(string name) : base (name)
        {
                
        }

        public void AddReqDtl(cinvReqDtlDispense ReqDtlNode)
        {
            if (lisReqDtl == null)
            {
                lisReqDtl = new List<cinvReqDtlDispense>();
            }
        ...
        public void AddRowSpread(FarPoint.Win.Spread.FpSpread spread)
        {
            ...
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.MOVE_REQ_NO, " ");
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.MOVE_DATE, MPCF.ToDate(this.MoveDate));
            spread.Sheets[0].SetTag(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.FROM_STORE, MPCF.Trim(this.FromStore));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.FROM_STORE, MPCF.Trim(this.FromStoreDesc));
            spread.Sheets[0].SetTag(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.TO_STORE, MPCF.Trim(this.ToStore));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.TO_STORE, MPCF.Trim(this.ToStoreDesc));
            spread.Sheets[0].SetTag(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.REASON, MPCF.Trim(this.ReasonDtl));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.REASON, MPCF.Trim(this.ReasonDtlDesc));
            spread.Sheets[0].SetTag(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.STATUS, BIGC.B_REQ_STATUS_REQUEST);
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqMst.STATUS, BIGC.B_REQ_STATUS_REQUEST_DESC);
        }
    ...
    public class cinvReqDtlDispense : TRSNode
    {               
        ...
        public List<cinvReqLotDispense> lisReqLot { get; set; }

        public cinvReqDtlDispense(string name) : base (name)
        {
            
        }

        public void AddReqLot(cinvReqLotDispense ReqLotNode)
        {
            if (lisReqLot == null)
            {
                lisReqLot = new List<cinvReqLotDispense>();
            }
        ...
        public void AddRowSpread(FarPoint.Win.Spread.FpSpread spread)
        {
            int row = 0;
            row = spread.Sheets[0].Rows.Count++;
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqDtl.MATERIAL, MPCF.Trim(this.MatId));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqDtl.MATERIAL_DESC, MPCF.Trim(this.MatDesc));            
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqDtl.UNIT, MPCF.Trim(this.Unit));            
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqDtl.WEIGHT_PINKING, MPCF.Trim(this.ReqQty1));
        ...
        {
            if(this.MatId == ((cinvReqDtlDispense)obj).Name) 
            {
 	            return true;
            }
            return false;
    ...
    public class cinvReqLotDispense : TRSNode
    {        
        ...
        public cinvReqLotDispense(string name) : base (name)
        {
        }

        public override bool Equals(object obj)
        {
            if(this.LotId == ((cinvReqLotDispense)obj).Name) 
            {
 	            return true;
            }
            return false;
        ...
        public void AddRowSpread(FarPoint.Win.Spread.FpSpread spread)
        {
            int row = 0;
            row = spread.Sheets[0].Rows.Count++;
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqLot.LOT_ID, MPCF.Trim(this.LotId));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqLot.MATERIAL, MPCF.Trim(this.MatId));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqLot.MATERIAL_DESC, MPCF.Trim(this.MatDesc));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqLot.UNIT, MPCF.Trim(this.Unit));
            spread.Sheets[0].SetValue(row, (int)frmINVPickingInventoryLotDispense.ColReqLot.QTY, MPCF.Trim(this.ReqQty1));
