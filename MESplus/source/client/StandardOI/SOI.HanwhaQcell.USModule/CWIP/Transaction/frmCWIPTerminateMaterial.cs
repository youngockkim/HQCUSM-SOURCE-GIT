...
namespace SOI.HanwhaQcell.USModule
{
    ...
    public partial class frmCWIPTerminateMaterial : SOIBaseForm02
    {
        #region Property
        ...
        public frmCWIPTerminateMaterial()
        {
            InitializeComponent();
        }
        ...
        private void frmCWIPTerminateMaterial_Shown(object sender, EventArgs e)
        {
            if (bIsShown == false)
            {
                ...
                MPCF.SetFocus(soiCodeProductionOrder);
        ...
        private void soiCodeProductionOrder_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                // In Node Setup
                ...
                soiCodeProductionOrder.Text = soiCodeProductionOrder.Show(soiCodeProductionOrder, "View Order List", "ORD", "ORD_View_Order_List", in_node, "ORD_LIST", display, header, "Order ID");

                // Validation
                if (string.IsNullOrEmpty(soiCodeProductionOrder.Text) == true)
                {
                    return;
                }
            }
        ...
        private void cdvMaterial_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                if (MPCF.CheckValue(soiCodeProductionOrder, false) == false)
                {
                    return;
                }
                ...
                // Display and Header Text Setup
                string[] display = new string[] { "MAT_ID", "MATE_TYPE", "MAT_DESC" };
                string[] header = new string[] { "Mat ID", "Mat Type", "Mat Desc" };

                // Show CodeView and Get Return
                cdvMaterial.Text = cdvMaterial.Show(cdvMaterial, "Order Material List", "CWIP", "CWIP_View_Order_BOM_List", in_node, "LIST", display, header, "Mat ID");
        ...
        private void cdvMaterialReasonCode_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                cdvMaterialReasonCode.Text = cdvMaterialReasonCode.Show(cdvMaterialReasonCode, "Code List", "BAS", "BAS_View_Data_List", in_node, "DATA_LIST", display, header, "KEY_1");
        ...
        private void spdTerminated_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            gSelectedMaterialOrderID = spdTerminated.Sheets[0].Cells[e.Row, (int)TERMINATED_LIST.ORDER_ID].Text;
            ...
            gSelectedMaterialMatDesc = (string)spdTerminated.Sheets[0].Cells[e.Row, (int)TERMINATED_LIST.MAT_ID].Tag;
            ...
            try
            {
                ...
                for (int i = 0; i < spdTerminated.Sheets[0].Rows.Count; i++)
                {
                    if (i == nRowSelect)
                    {
                        ...
                        gSelectedMaterialOrderID = spdTerminated.Sheets[0].Cells[i, (int)TERMINATED_LIST.ORDER_ID].Text;
