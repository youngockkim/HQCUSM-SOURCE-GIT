...
namespace SOI.HanwhaQcell.USModule
{
    ...
    public partial class frmCUSModulePackingLabel : SOIBaseForm03
    {
        #region Property
        ...
        public frmCUSModulePackingLabel()
        {
            InitializeComponent();
        }
        ...
        private bool ProcPrint_v1(TRSNode out_node)
        {
            ...
            string reportID = GetReportID(out_node.GetString("MAT_ID"), grade);
        ...
        private bool PackLotListAdd(string PalletID)
        {
            try
            {
                MPCF.ClearList(spdList);
                ...
                if (out_node.GetList(0).Count > 0)
                {
                    for (int i = 0; i < out_node.GetList(0).Count; i++)
                    {
                        ...
                        */
                        spdList.ActiveSheet.AddRows(i, 1);
                        spdList.ActiveSheet.Cells[i, (int)LOT_LIST.SEQ].Value = i+1;// out_node.GetList(0)[i].GetInt("SEQ");
                        spdList.ActiveSheet.Cells[i, (int)LOT_LIST.ARTICLE_NO].Value = out_node.GetList(0)[i].GetString("ARTICLE_NO");
                        spdList.ActiveSheet.Cells[i, (int)LOT_LIST.BATCH_NO].Value = out_node.GetList(0)[i].GetString("BATCH_NO");
                        spdList.ActiveSheet.Cells[i, (int)LOT_LIST.PRODUCT].Value = out_node.GetList(0)[i].GetString("POWER_CLASS");
                        spdList.ActiveSheet.Cells[i, (int)LOT_LIST.TYPE].Value = out_node.GetList(0)[i].GetChar("GRADE");
                        spdList.ActiveSheet.Cells[i, (int)LOT_LIST.BARCODE].Value = out_node.GetList(0)[i].GetString("MODULE_ID");
        ...
        {
            string reportID = "";
            //bool isGradeA = (grade == "G01" || grade == "G02" || grade == "A" ? true : false);
