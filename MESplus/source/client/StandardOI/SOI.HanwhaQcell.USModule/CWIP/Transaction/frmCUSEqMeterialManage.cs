...
namespace SOI.HanwhaQcell.USModule
{
    ...
    public partial class frmCUSEqMeterialManage : SOIBaseForm02
    {
        #region Property
        ...
        private void cdvMatID_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                MPCF.SetInMsg(in_node);
                in_node.ProcStep = '1';
                in_node.AddString("MAT_ID", MPCF.Trim(cdvMatID.Code));
                ...
                cdvMatID.Code = cdvMatID.Show(cdvMatID, "View Material List", "WIP", "WIP_View_Material_List", in_node, "LIST", display, header, "MAT_ID");
        ...
        private bool ViewOrderBOMList()
        {
            try
            {
                ...
                MPCF.SetInMsg(in_node);
                in_node.ProcStep = '1';
                //in_node.AddString("ORDER_ID", MPCF.Trim(strOrderNo));

                in_node.AddString("BOM_SET_ID", MPCF.Trim(cdvMatID.Code));
                in_node.AddInt("BOM_SET_VERSION", 1);
                //in_node.AddString("OPER", MPCF.Trim(cdvOperID.Code));

                if (MPCF.CallService("CWIP", "CWIP_View_BOM_List", in_node, ref out_node) == false)
                {
                    return false;
                }
