...
namespace SOI.HanwhaQcell.USModule
{
    ...
    public partial class frmCWIPTerminateModules_V2 : SOIBaseForm02
    {
        ...
        private string gTerminatedEqpCode = string.Empty;
        ...
        private void cdvTerminateProcess_CodeViewButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(gSelectedOper))
                {
                    in_node.AddString("KEY_1", " ");
                }
                else if (Convert.ToInt32(gSelectedOper.Substring(1)) >= 3000)
                {
                    in_node.AddString("KEY_1", "P0102");
                }
                else if (Convert.ToInt32(gSelectedOper.Substring(1)) < 3000)
                {
                    in_node.AddString("KEY_1", "P0103");
                }
                else
                {
                    in_node.AddString("KEY_1", " ");
        ...
        private bool SendCancalTerminationToERP()
        {

            TRSNode in_node = new TRSNode("CANCEL_TERMINATION_IN");
            ...
            try
            {
                ...
                in_node.AddString("CANCEL_FLAG", "Y");

                if (MPCF.CallService("CWIP", "CWIP_Update_Map_Data_To_ERP_IF", in_node, ref out_node) == false)
                {
                    return false;
                }
        ...
        private void spdModule_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            if (spdModule.Sheets[0].RowCount > 0)
            {
                ...
                gTerminatedEqpCode = spdModule.Sheets[0].Cells[e.Row, (int)LOT_LIST.RES_ID].Text;
        ...
        private void gsClear()
        {
            gSelectedLotID = string.Empty;
            gSelectedMatID = string.Empty;
            gSelectedFlow = string.Empty;
            gSelectedOper = string.Empty;
            //gSelectedLastTranTime;
            gSelectedMatVer = 1;
            gSelectedFlowSeqNum = 1;
            gTerminatedLotID = string.Empty;
            gTerminatedLastActiveHisSeq = 0;
            cdvProcess.Text = string.Empty;
            cdvProcess.Description = string.Empty;
            cdvTerminateCode.Text = string.Empty;
            cdvTerminateCode.Description = string.Empty;
            txtNumOfStrings.Text = string.Empty;
            txtComment.Text = string.Empty;
            gTerminatedEqpCode = string.Empty;
        ...
        private bool ViewLotList()
        {
            try
            {
                ...
                for (i = 0; i < out_node.GetList(0).Count; i++)
                {
                    ...
                    spdModule.ActiveSheet.Cells[i, (int)LOT_LIST.RES_ID].Value = lot_list.GetString("RES_ID");
        ...
        private bool TerminateLot()
        {
            ...
            try
            {
                ...
                in_node.AddString("TERMINATE_EQ_CODE", gTerminatedEqpCode);
