...
namespace SOI.HanwhaQcell.USModule
{
    ...
    public partial class frmCUSModuleRepairManagementOverkill : SOIBaseForm02
    {
        #region Property
        ...
        public frmCUSModuleRepairManagementOverkill()
        {
            InitializeComponent();
                    }
        ...
        private bool Module_Loss_Input()
        {
            ...
            try
            {
				...
				in_node.AddInt("OVERKILL_ENABLE", nUseOverkill);        //Worker2
                ...
                if (MPCF.CallService("CWIP", "CWIP_Update_Module_Repair_Overkill", in_node, ref out_node) == false)
                {
                    return false;
                }
        ...
        private bool ViewLoss(string sLotId)
        {
            try
            {
                ...
                m_strELImagePath = out_node.GetString("EL_IMAGE_PATH");
                m_strFAOIImagePath = out_node.GetString("AOI_IMAGE_PATH");
                ...
                if (m_strELImagePath.IndexOf(nassrv1.First().Value) > 0)
                {
                    m_strELImagePath = m_strELImagePath.Replace("\\\\" + nassrv1.First().Value, strEMIAddr).Replace("\\", "/");
                    if (nassrv1.Count() > 0 && emivir1.Count() > 0)
                    {
                        strVirtualDir = emivir1.First().Value;
                    }
                }
                else if (m_strELImagePath.IndexOf(nassrv2.First().Value) > 0)
                {
                    m_strELImagePath = m_strELImagePath.Replace("\\\\" + nassrv2.First().Value, strEMIAddr).Replace("\\", "/");
                    if (nassrv2.Count() > 0 && emivir1.Count() > 0 && emivir2.Count() > 0)
                    {
                        strVirtualDir = emivir2.First().Value;
                        m_strELImagePath = m_strELImagePath.Replace(emivir1.First().Value, emivir2.First().Value);
                    }
                }

                if (m_strFAOIImagePath.IndexOf(nassrv1.First().Value) > 0)
                {
                    m_strFAOIImagePath = m_strFAOIImagePath.Replace("\\\\" + nassrv1.First().Value, strEMIAddr).Replace("\\", "/");
                    if (nassrv1.Count() > 0 && emivir1.Count() > 0)
                    {
                        strVirtualDir = emivir1.First().Value;
                    }
                }
                else if (m_strFAOIImagePath.IndexOf(nassrv2.First().Value) > 0)
                {
                    m_strFAOIImagePath = m_strFAOIImagePath.Replace("\\\\" + nassrv2.First().Value, strEMIAddr).Replace("\\", "/");
                    if (nassrv2.Count() > 0 && emivir1.Count() > 0 && emivir2.Count() > 0)
                    {
                        strVirtualDir = emivir2.First().Value;
                        m_strFAOIImagePath = m_strFAOIImagePath.Replace(emivir1.First().Value, emivir2.First().Value);
                    }
                }
                ...
                m_nas_conn_failed = out_gcm_node.GetList(0)[0].GetString("DATA_1");
                if (m_nas_conn_failed.Equals("0"))
                {
                ...
                else if (m_nas_conn_failed.Equals("1"))
                {
                ...
                {
                    m_strELImagePath = strRejectImagePath;
                }

				// IS-21-11-013 [신규]모듈 리페어 과검 프로세스 도입
				// chkOverKill
				int nFindOverKillValue = 0;
				int nFindOverKillUse = 0;
				nFindOverKillValue = out_node.GetInt("OVERKILL_VALUE");
				nFindOverKillUse = out_node.GetInt("OVERKILL_ENABLE");
				if (nFindOverKillValue == 1)
				{
					chkOverKill.Checked = true;
				}
				else
				{
				...
				if (nFindOverKillUse == 1)
				{
					chkOverKill.Visible = true;
				}
				else
				{
        ...
        private void soiButton1_Click(object sender, EventArgs e)
        {
            try
            {
                if (m_strELImagePath != "")
                {
                    ...
                    {
                        frmCWIPFullCellImagePopup pop = new frmCWIPFullCellImagePopup(m_strELImagePath);
        ...
        private void soiButton1_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (m_strELImagePath.Equals("") || m_strELImagePath.Equals(" "))
                {
        ...
        /*
        private void soiRejudgeButton_Click(object sender, EventArgs e)
        {
            TRSNode in_node = new TRSNode("MODULE_SUALAB_REJUDGE_IN");
            TRSNode out_node = new TRSNode("MODULE_SUALAB_REJUDGE_OUT");
            //soiRejudgeButton.Enabled = false;
            //soiRejudgeButton.Visible = false;
            try
            {
                MPCF.SetInMsg(in_node);
                ...
                in_node.AddString("LOT_ID", strLotId);
                in_node.AddString("LINE_ID", strLineId);
                in_node.AddString("CLIENT_ID", in_node.UserID);

                in_node.AddString("RESULT", "0");
                //in_node.AddString("MODE", "1");
                in_node.AddString("MODE", strModeCode);
                in_node.AddString("RES_ID", strRedId);
                in_node.AddString("TYPE_FLAG", "2");   // '1': Inspected by Equipment, '2': Inspected by Workers

                if (MPCF.CallService("CEIS", "EAPMES_Collect_Inspection_Data_Sualab", in_node, ref out_node) == false)
                {
                    return ;
                }
                ...
                MPCF.ShowMessage(MPCF.GetMessage(52), MSG_LEVEL.INFO, false);
                return ;
            }
            catch (Exception ex)
            {
        ...
        private void btnELImage_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(txtLotID.Text) == true)
                ...
                else// if(m_strELImagePath != "")
                {
                    ...
                    {
                        //local에서 테스트시에만 사용
                        //if (m_strELImagePath.IndexOf("el_img") > 0)
                        //{
                        //    m_strELImagePath = m_strELImagePath.Replace("el_img", "el_img2");
                        //}
                        //local에서 테스트시에만 사용
        ...
        private void btnFAOIImage_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(txtLotID.Text) == true)
                ...
                if (m_strFAOIImagePath.Equals("") || m_strFAOIImagePath.Equals(" "))
                {
                    MPCF.ShowMsgBox(MPCF.GetMessage(551) + Environment.NewLine + MPCF.FindLanguage("[MODULE ID]"));
                    MPCF.SetFocus(txtLotID);
                    return;
                }
                else// if(m_strFAOIImagePath != "")
                {
                    if (m_nas_conn_failed.Trim().Equals("1"))
                    {
                        MPCF.ShowMessage("EL image cannot be displayed due to connection issues. The rest module repair steps on this page can still be proceeded.", MSG_LEVEL.ERROR, false);
                    ...
                    {
                        //local에서 테스트시에만 사용
                        //if (m_strFAOIImagePath.IndexOf("AOI_img") > 0)
                        //{
                        //    m_strFAOIImagePath = m_strFAOIImagePath.Replace("AOI_img", "AOI_img2");
                        //}
                        //local에서 테스트시에만 사용

                        frmCWIPFullCellImagePopup pop = new frmCWIPFullCellImagePopup(m_strFAOIImagePath);
                        pop.Owner = MPGV.gfrmMDI;
                        pop.ShowDialog();
                    }
                }
