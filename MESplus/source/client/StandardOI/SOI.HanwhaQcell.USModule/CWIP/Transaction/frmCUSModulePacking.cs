...
namespace SOI.HanwhaQcell.USModule
{
    ...
    public partial class frmCUSModulePacking : SOIBaseForm02
    {
        #region Property
        ...
        private void txtModuleID_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                ...
                if (e.KeyChar == (char)13)
                {
                    ...
                    {
                        if (ViewPackingLotInfo(txtModuleID.Text) == false)
                        {
                            txtModuleID.Focus();
                            txtModuleID.SelectAll();
                            return;
                    ...
                    if (ViewFQCInfo(txtModuleID.Text, out_node))
                    {
                        if (CheckSpec() == false)
                        {
                    ...
                    ModuleListAdd(txtModuleID.Text, out_node);
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            frmCWIPManualHVPopup f = new frmCWIPManualHVPopup();
            f.Owner = MPGV.gfrmMDI;
            drResult = f.ShowDialog();

            // H / V 선택
            if (drResult == DialogResult.OK) //수직 V
            {
                packHV = "V";
            }else if (drResult == DialogResult.No) //수평 H
            {
                packHV = "H";
            }
            else
        ...
        private void resetPalletID()
        {
            int row = spdLotList.ActiveSheet.ActiveRowIndex;
            String module_id = txtModuleID.Text;
            TRSNode out_node = new TRSNode("VIEW_FQC_OUT");

            if (spdLotList.ActiveSheet.ActiveRowIndex > 0)
            {
                row = spdLotList.ActiveSheet.ActiveRowIndex - 1;
            }

            module_id = spdLotList.ActiveSheet.Cells[row, (int)LOT_LIST.MODULE_ID].Value.ToString();

            if (ViewPackingLotInfo(module_id) == false)
            {
                txtModuleID.Focus();
                txtModuleID.SelectAll();
                return;
            ...
            if (!ViewFQCInfo(module_id, out_node))
            {
                return;
            } 
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (spdLotList.ActiveSheet.ActiveRowIndex == spdLotList.ActiveSheet.RowCount-1)
                {
                    this.resetPalletInfo();
                    this.resetPalletID();
                }
        ...
        */
        ...
        private bool ViewFQCInfo(string sLotID, TRSNode out_node)
        {
            try
            {
                ...
                in_node.AddString("IN_MATID", cdvMatID.Text);
                ...
                //CWIP_View_Fqc_Result 에서 주는 MAT_ID 값으로 변경. 왜 WIP_View_Lot 으로 재조회 했는지 모르겠음.
        ...
        private bool ViewPackingLotInfo(string sLotID)
        {
            try
            {
                ...
                if (MPCF.CallService("CWIP", "CWIP_View_Packing_Lot", in_node, ref out_node, false) == false)
                {
                    MPCF.ShowMsgBox(out_node.Msg);
                    return false;
                }

                cdvLineID.Text = MPCF.Trim(out_node.GetString("LINE_ID").ToString());
                cdvOperID.Text = MPCF.Trim(out_node.GetString("OPER").ToString());
                cdvEquipID.Text = MPCF.Trim(out_node.GetString("RES_ID").ToString());
                cdvMatID.Text = MPCF.Trim(out_node.GetString("MAT_ID").ToString());
                txtMatDesc.Text = MPCF.Trim(out_node.GetString("MAT_DESC").ToString());
                cdvGrade.Text = MPCF.Trim(out_node.GetString("GRADE").ToString());
                cdvPower.Text = MPCF.Trim(out_node.GetString("POWER").ToString());
                cdvModuleType.Text = MPCF.Trim(out_node.GetString("MODULE_TYPE").ToString());

                SetArticleNoVisible( out_node.GetString("MAT_ID").ToString(),
                    out_node.GetString("ARTICLE_NO").ToString(), out_node.GetString("ARTICLE_DESC").ToString());
                if (this.isNewMaterialID(MPCF.Trim(out_node.GetString("MAT_ID").ToString())))
                {
                    txtBarcode.Text = this.SetBarcodeGrade(MPCF.Trim(out_node.GetString("BARCODE").ToString()), 
                        MPCF.Trim(out_node.GetString("GRADE").ToString()),
                        MPCF.Trim(out_node.GetString("POWER").ToString()));
                }
                else
                {
                    txtBarcode.Text = MPCF.Trim(out_node.GetString("BARCODE").ToString());
                }

                txtPowerClass.Text = MPCF.Trim(out_node.GetString("POWER_CLASS").ToString()); 
                txtLimitCount.Text = MPCF.Trim(out_node.GetInt("MODUL_QTY").ToString());
        ...
        private bool ViewPackingInfo()
        {
            try
            {
                ...
                if (this.isNewMaterialID(MPCF.Trim(cdvMatID.Text)))
                {
                    //GRADE정보 BARCODE에 반영(2023/04/17,ERP고도화)
                    txtBarcode.Text = this.SetBarcodeGrade(out_node.GetString("DATA_2"), this.cdvGrade.Value, this.cdvPower.Value);
                }
                else
                {
                    txtBarcode.Text = out_node.GetString("DATA_2");
                }
        ...
        private bool ModuleListAdd(string sLotID, TRSNode out_node)
        {
            try
            {
                ...
                spdLotList.ActiveSheet.Cells[0, (int)LOT_LIST.MAT_ID].Value = out_node.GetString("MAT_ID");
//                spdLotList.ActiveSheet.Cells[0, (int)LOT_LIST.MAT_ID].Value = cdvMatID.Text;
        ...
        private bool Tran_Pack_Lot_Input()
        {
            ...
            TRSNode in_valid_node = new TRSNode("TRAN_IN");
            TRSNode out_valid_node = new TRSNode("TRAN_OUT");
            TRSNode gcm_item;
            ...
            try
            {
                ...
                in_node.SetString("CLIENT_IP", HQGV.gsClientIPAddress);
                in_node.SetString("CLIENT_VERSION", MPGV.gsClientVersion);
                in_node.SetString("FUNC_NAME", this.menuInfo.s_func_name.ToString());
                in_node.SetString("PACK_DIRECTION", MPCF.Trim(packHV)); //pack_type V or H 팝업에서 결정
                in_node.SetString("AUTO_COMPLETE", " "); // ' ' Manual Packing
                in_node.SetString("PALLET_ID", txtPackingID.Text);
                ...
                for (int k = spdLotList.ActiveSheet.RowCount - 1; k > -1; k--)
                {
                    ...
                    list_item.AddString("CMF_5", MPCF.Trim(packHV));
                }

                // Call Service
                MPCF.SetInMsg(in_valid_node);
                in_valid_node.ProcStep = '1';
                in_valid_node.AddString("TABLE_NAME", "@VALIDATION_RULE");
                in_valid_node.AddString("KEY_1", "LIMIT_PACKING");

                if (MPCF.CallService("BAS", "BAS_View_Data_List", in_valid_node, ref out_valid_node) == false)
                {
                    return false;
                }

                //LIMIT PACKING : Y 이면 무조건 셋팅된 LimitCount 와 일치, N 이면 기존처럼 수량 체크 물어보고 로직 넘어갈수 있도록 수정
                gcm_item = out_valid_node.GetList(0)[0];
                if (gcm_item.GetString("DATA_2") == "Y")
                {
                    if (spdLotList.ActiveSheet.RowCount != MPCF.ToInt(MPCF.Trim(txtLimitCount.Text)))
                    {
                        MPCF.ShowMessage(MPCF.GetMessage(552), MSG_LEVEL.INFO, false);
                        return false;
                    }
                }
                else
                {
                    if (MPCF.ShowMsgBox(MPCF.GetMessage(516), MessageBoxButtons.YesNo, MSG_LEVEL.WARNING) != System.Windows.Forms.DialogResult.Yes)
                    {
                        return false;
                    }
                }
        ...
        private bool procprint_v1()
        {
            ...
            string reportID = GetReportID(strMaterial, cdvGrade.Text);
            ...
            try
            {
                ...
                if (out_node.GetList(0).Count > 0)
                {
                    ...
                    for (i = 0; i < out_node.GetList(0).Count; i++)
                    {
                        ...
                        //isGradeA = (grade == "G01" || grade == "G02" || grade == "A" ? true : false);
