...
namespace SOI.ATP
{
    ...
    public partial class frmATPRejectReceipt : SOIBaseForm02
    {
        #region Property
        ...
        public ATPRejectReceiptModel model = new ATPRejectReceiptModel();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnDeleteScanLot_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        private bool AddToScanList(string invLotID)
        {
            try
            {
                ...
                spdScanList.Sheets[0].Cells[iRow, (int)COL_SCAN_LIST.INV_MAT_ID].Value = out_node.GetString("INV_MAT_ID");
            ...
            try
            {
                ...
                for (int i = 0; i < spdSumInfo.Sheets[0].Rows.Count; i++)
                {
                    if (spdSumInfo.Sheets[0].Cells[i, (int)COL_SUM_LIST.INV_MAT_ID].Value.ToString() == invMatID)
                    {
                        // Remain Qty - Scan Qty
                        spdSumInfo.Sheets[0].Cells[i, (int)COL_SUM_LIST.REMAIN_QTY].Value =
                            (MPCF.ToDbl(spdSumInfo.Sheets[0].Cells[i, (int)COL_SUM_LIST.REMAIN_QTY].Value) + scanQty).ToString();

                        // Pre-Scan Qty + Scan Qty
                        spdSumInfo.Sheets[0].Cells[i, (int)COL_SUM_LIST.SCAN_QTY].Value =
                            (MPCF.ToDbl(spdSumInfo.Sheets[0].Cells[i, (int)COL_SUM_LIST.SCAN_QTY].Value) - scanQty).ToString();
        ...
        /// <summary>
        /// Process Reject Receipt
        /// </summary>
        /// <returns></returns>
        private bool RejectReceipt()
        {
            try
            {
                ...
                {
                    in_node.AddChar("CANCEL_REJECT_FLAG", 'Y');
                }
        ...
        /// </summary>
        /// <returns></returns>
        private bool RejectReceiptRefresh()
        {
            try
            {
                // Clear
    ...
    public class ATPRejectReceiptModel
    {
