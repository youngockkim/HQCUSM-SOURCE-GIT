...
namespace BOI.WIPCus
{
    ...
    public partial class frmWIPInputMaterialOperPretreatment : BOIBaseForm02
    {
        #region struct
        struct MATLOTINFO
        {
        ...
        public frmWIPInputMaterialOperPretreatment()
        {
            InitializeComponent();
            OrderHeaderChange();
        }

        public frmWIPInputMaterialOperPretreatment(string args)
        {
            InitializeComponent();
            _orderId = args;
            OrderHeaderChange();
        ...
        private void frmTempBOIBaseForm02_Shown(object sender, EventArgs e)
        {
            ...
            if (bIsShown == false)
            {
                ...
                if (_lotId != string.Empty)
                {
                    ...
                    if (boiLotInfo.spdLotInfo.Sheets[0].RowCount > 0)
                    {                        
                        ViewBomList(_matId, _matBomType, _oper, _orderId, _lotId);
                        if (_lotId != string.Empty)
                        {
                            ViewAssemblyList(_lotId, _oper);
                        }
        ...
        private void frmWIPInputMaterialOperPretreatment_Com_1_DataReceived(string aData)
        {
            try
            {
        ...
        private void ViewBomList(string matId, string matBomType, string oper, string orderId, string lotId)
        {
            ...
            try
            {
                ...
                dvcArgu[3].sCondtion_ID = "OPER";
                dvcArgu[3].sCondition_Value = oper;

                dvcArgu[4].sCondtion_ID = "ORDER_ID";
                dvcArgu[4].sCondition_Value = orderId;

                dvcArgu[5].sCondtion_ID = "LOT_ID";
                dvcArgu[5].sCondition_Value = lotId;
