...
namespace BOI.WIPCus.Popup
{
    ...
    public partial class frmWIPCommonWork : frmPopupBase
    {
        private enum WORKER
        {
            CHECK = 0,
        ...
        public frmWIPCommonWork(string sWorkDate, string sTeam, string sTeamDesc, string sPart, string sPartDesc, string[] user_id, string[] user_desc, string[] common_work, string[] common_work_desc, string[] common_work_time, string[] common_work_time_desc)
        {
            InitializeComponent();
            txtTeam.Tag = sTeam;
            ...
            for (int i = 0; i < user_id.Length; i++)
            {
                ...
                spdWorkerList.ActiveSheet.Cells[i, (int)WORK_MANAGE.COMMON_WORK].Tag = common_work[i];
                spdWorkerList.ActiveSheet.Cells[i, (int)WORK_MANAGE.COMMON_WORK].Text = common_work_desc[i];
                spdWorkerList.ActiveSheet.Cells[i, (int)WORK_MANAGE.COMMON_WORK_TIME].Tag = common_work_time[i];
                spdWorkerList.ActiveSheet.Cells[i, (int)WORK_MANAGE.COMMON_WORK_TIME].Text = common_work_time_desc[i];
            }
        }
        ...
        private bool Update_Work_Status(char cProcStep)
        {
            ...
            for (i = 0; i < spdWorkerList.Sheets[0].RowCount; i++)
            {
                ...
                if (Convert.ToBoolean(spdWorkerList.ActiveSheet.Cells[i,(int)WORK_MANAGE.CHECK].Value) == true)
                {
                    ...
                    node.SetChar("MODE", 'M');//공통 업무 처리
                    if (MPCF.Trim(spdWorkerList.ActiveSheet.Cells[i, (int)WORK_MANAGE.COMMON_WORK].Tag.ToString()) != "")
                    {
                        node.SetChar("COMMON_WRK_FLAG", 'Y');
                        node.SetString("COMMON_WRK_RSN", MPCF.Trim(spdWorkerList.ActiveSheet.Cells[i, (int)WORK_MANAGE.COMMON_WORK].Tag.ToString()));
                        node.SetInt("COMMON_WRK_TIME", MPCF.ToInt(spdWorkerList.ActiveSheet.Cells[i, (int)WORK_MANAGE.COMMON_WORK_TIME].Tag.ToString()));
                    }
                }
                else
                {
        ...
        private void spdAddList_ButtonClicked(object sender, FarPoint.Win.Spread.EditorNotifyEventArgs e)
        {
            try
            {
                if (e.Column == (int)WORK_MANAGE.COMMON_WORK_BTN)
                {
                    ...
                    // Show
                    cdvLateTime.Text = cdvLateTime.Show(cdvLateTime, "Code Desc", "OWIP2503-006", dvcArgu, display, header, "DATA_1", -1);

                    if (MPCF.Trim(cdvWorkTime.Text) != "")
                    {
                        if (cdvLateTime.returnDatas != null && cdvLateTime.returnDatas.Count > 0)
                        {
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK].Tag = cdvLateTime.returnDatas[0];
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK].Text = cdvLateTime.returnDatas[1];
                        ...
                        {
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK].Tag = "";
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK].Text = "";
                        }
                    }
                    else
                    {
                ...
                else if (e.Column == (int)WORK_MANAGE.COMMON_WORK_TIME_BTN)
                {
                    ...
                    if (MPCF.Trim(cdvWorkTime.Text) != "")
                    {
                        ...
                        {
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK_TIME].Tag = cdvWorkTime.returnDatas[0];
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK_TIME].Text = cdvWorkTime.returnDatas[1];
                            spdWorkerList.ActiveSheet.Rows[e.Row].BackColor = Color.Salmon;
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.CHECK].Value = true;
                        }
                        else
                        {
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK_TIME].Tag = "";
                            spdWorkerList.ActiveSheet.Cells[e.Row, (int)WORK_MANAGE.COMMON_WORK_TIME].Text = "";
                        }
                    }
                    else
                    {
