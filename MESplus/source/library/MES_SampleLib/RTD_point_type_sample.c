...
int RTD_Point_Type_1(TRSNode *in_node, TRSNode *out_node)
{
    struct MWIPLOTSTS_TAG MWIPLOTSTS;

    LOG_head("RTD_Point_Type_1");
    ...
    if(MWIPLOTSTS.LOT_PRIORITY <= '4')
    {
        TRS.set_char(out_node, "RESULT_FLAG", 'Y');

        TRS.set_nstring(out_node, "UNSELECT_REASON","UNSELECT REASON : LOT_PRIORITY IS LOW");
        TRS.set_nstring(out_node, "UNCAPABLE_REASON","UNCAPABLE REASON : LOT_PRIORITY IS LOW");
        TRS.set_nstring(out_node, "PDS_CMF_1","CMF 1");
    }
    else
    {
        TRS.set_char(out_node, "RESULT_FLAG", 'N');
...
int RTD_Point_Type_Qty_1(TRSNode *in_node, TRSNode *out_node)
{
    struct MWIPLOTSTS_TAG MWIPLOTSTS;

    LOG_head("RTD_Point_Type_Qty_1");
    LOG_add("LOT_ID", MP_NSTR, TRS.get_string(in_node, "LOT_ID"));
    LOG_add("RES_ID", MP_NSTR, TRS.get_string(in_node, "RES_ID"));
    LOG_add("KEY_VALUE_1", MP_NSTR, TRS.get_string(in_node, "KEY_VALUE_1"));
    LOG_add("KEY_VALUE_2", MP_NSTR, TRS.get_string(in_node, "KEY_VALUE_2"));
    ...
    if(MWIPLOTSTS.QTY_1 > 90.0)
    {
        TRS.set_char(out_node, "RESULT_FLAG", 'Y');

        TRS.set_nstring(out_node, "UNSELECT_REASON","UNSELECT REASON : QTY_1 is over 90");
        TRS.set_nstring(out_node, "UNCAPABLE_REASON","UNCAPABLE REASON : QTY_1 is over 90");
        TRS.set_nstring(out_node, "PDS_CMF_2","CMF 2");
    }
    else
    {
        TRS.set_char(out_node, "RESULT_FLAG", 'N');
