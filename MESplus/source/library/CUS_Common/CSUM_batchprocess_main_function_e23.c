...
File Name   : CSUM_BatchProcess_MainFunction_E23.c
...
int CSUM_BATCHPROCESS_TRANSACTION_E23(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int CSUM_BatchProcess_Transaction_E23_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);

/*******************************************************************************
CSUM_BatchProcess_Transaction_E23()
- End Lot
Return Value
- int : 0 (MP_TRUE)
Arguments
...
*******************************************************************************/
int CSUM_BatchProcess_Transaction_E23(TRSNode *in_node, TRSNode *out_node)
{
	...
	i_ret = CSUM_BATCHPROCESS_TRANSACTION_E23(s_msg_code, in_node, out_node);

	COM_out_msg_log_write(s_msg_code, "CSUM_BATCHPROCESS_TRANSACTION_E23", out_node);
...
/*******************************************************************************
CSUM_BATCHPROCESS_TRANSACTION_E23()
- Main sub function of "CSUM_BatchProcess_Transaction_E23" function
Return Value
...
int CSUM_BATCHPROCESS_TRANSACTION_E23(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    ...
    LOG_head("CSUM_BATCHPROCESS_TRANSACTION_E23");
	...
	while(1)
	{
		...
		else if (memcmp(MTMPLOTHIS.TRAN_CODE, "COMBINE", strlen("COMBINE")) == 0)
		{
			if (CSUM_SUMMARY_WIP_TRAN_COMBINELOT(s_msg_code, &MTMPLOTHIS, in_node, out_node) == MP_FALSE)
			{
				//ERROR 
				DB_rollback();
				//[23.09.05] 데이터 D 플래그 처리
				//스트링 값이 12개 이상으로 저장 되는 현상
				DBC_update_mtmplothis(2, &MTMPLOTHIS);
				//[23.09.05] 데이터 D 플래그 처리
		...
		else if (memcmp(MTMPLOTHIS.TRAN_CODE, "TERMINATE", strlen("TERMINATE")) == 0)
		{
			...
			{
				//Bug Fix
				TRS.set_char(in_node, "CANCEL_FLAG", 'N');
			}
...
/*******************************************************************************
CSUM_BatchProcess_Transaction_E23_Validation()
- Validation check sub function of "CSUM_BatchProcess_Transaction" function
...
- TRSNode *out_node : Output Message structure
*******************************************************************************/
int CSUM_BATCHPROCESS_EOH_E23(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	struct worktime_tag cur_work_time;
	struct worktime_tag prev_work_time;
	
	char s_sys_time[14];
	char s_prev_time[14];

	struct RSUMWIPEOH_TAG RSUMWIPEOH;
	//GET FACTORY SHIFT TIME
	
	memset(&cur_work_time, 0x00, sizeof(struct worktime_tag));
	memset(&prev_work_time, 0x00, sizeof(struct worktime_tag));

    CCOM_get_work_time(s_sys_time, &cur_work_time);

	COM_sub_time_sec(s_prev_time, s_sys_time, 60 * 60);
	//
	CCOM_get_work_time(s_prev_time, &prev_work_time);

	CDB_init_rsumwipeoh(&RSUMWIPEOH);
	memcpy(RSUMWIPEOH.WORK_DATE, cur_work_time.work_date, sizeof(RSUMWIPEOH.WORK_DATE));
	RSUMWIPEOH.SHIFT = (char)(cur_work_time.work_shift + '0');
	if((int)CDB_select_rsumwipeoh_scalar(2, &RSUMWIPEOH) <= 0)
	{
		CDB_init_rsumwipeoh(&RSUMWIPEOH);
		memcpy(RSUMWIPEOH.WORK_DATE, cur_work_time.work_date, sizeof(RSUMWIPEOH.WORK_DATE));
		COM_itoa_left(RSUMWIPEOH.WORK_MONTH, cur_work_time.work_month, sizeof(RSUMWIPEOH.WORK_MONTH));
		COM_itoa_left(RSUMWIPEOH.WORK_WEEK, cur_work_time.work_week, sizeof(RSUMWIPEOH.WORK_WEEK));
		COM_itoa_left(RSUMWIPEOH.WORK_DAYS, cur_work_time.work_days, sizeof(RSUMWIPEOH.WORK_DAYS));
		RSUMWIPEOH.WORK_DAY_OF_WEEK = (char)(cur_work_time.work_day_of_week +  '0');
		RSUMWIPEOH.SHIFT = (char)(cur_work_time.work_shift + '0');

		//이전 SHIFT EOH 값을 현재 SHIFT EOH 값으로 COPY 함.
		CDB_shift_change_rsumwipeoh(101, &RSUMWIPEOH, prev_work_time.work_date, (char)(prev_work_time.work_shift + '0'), cur_work_time.work_date_time);
		if(DB_error_code != DB_SUCCESS)
		{ 		
			return MP_FALSE;
		}
