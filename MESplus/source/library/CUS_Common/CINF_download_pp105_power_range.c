/*******************************************************************************
    ...
    File Name   : CINF_download_pp105_power_range.c
...
int CUS_INTERFACE_DOWNLOAD_POWER_RANGE(char *s_msg_code,
                        TRSNode *in_node, 
                        TRSNode *out_node);
...
*******************************************************************************/
int CUS_Interface_Download_Power_Range(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CUS_INTERFACE_DOWNLOAD_POWER_RANGE(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CUS_Interface_Download_Power_Range", out_node);
...
/*******************************************************************************
    CUS_INTERFACE_DOWNLOAD_POWER_RANGE()
        - ERP->MES Power Range
    return Value
        - int : 0 (MP_TRUE)
    Arguments
	...
	while(1) 
    {
		...
		if(BAS_UPDATE_DATA_LIST(s_msg_code, in_node1, out_node1)== MP_FALSE)
		{
			...
			*/
			...
			COM_out_msg_log_write(s_msg_code, "CUS_Interface_Download_Power_Range", out_node1);
