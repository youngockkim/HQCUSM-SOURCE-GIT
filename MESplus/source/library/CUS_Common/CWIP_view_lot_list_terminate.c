/******************************************************************************'
    ...
    System      : MESplus
    Module      : CWIP
    File Name   : CWIP_view_lot_list_terminate.c
    Description : View Lot List

    MES Version : 5.3.6.4

    Function List
        - CWIP_View_Lot_List_Terminate()
            + View Lot List
        - CWIP_VIEW_LOT_LIST_TERMINATE()
            + Main sub function of CWIP_View_Lot_List function
            + View Lot List definition
        - CWIP_View_Lot_List_Validation_Terminate()
            + Main sub function of CWIP_VIEW_LOT_LIST function
...
*******************************************************************************/
int CWIP_View_Lot_List_Terminate(TRSNode* in_node, TRSNode* out_node)
{
    ...
    i_ret = CWIP_VIEW_LOT_LIST_TERMINATE(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CWIP_VIEW_LOT_LIST_TERMINATE", out_node);
...
/*******************************************************************************
    CWIP_VIEW_LOT_LIST()
        - Main sub function of "CWIP_View_Lot_List_Terminate" function
        - View Lot List
    Return Value
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CWIP_VIEW_LOT_LIST_TERMINATE(char* s_msg_code, TRSNode* in_node, TRSNode* out_node)
{
    ...
    LOG_head("CWIP_VIEW_LOT_LIST_TERMINATE");
    ...
    */
    ...
    if (CWIP_View_Lot_List_Validation_Terminate(s_msg_code, in_node, out_node) == MP_FALSE)
    {
    ...
    while (1)
    {
        ...
        if (TRS.get_procstep(in_node) == '3' || TRS.get_procstep(in_node) == '4' || TRS.get_procstep(in_node) == '6') // [ERP 23.05.24] Step 추가
        {
            ...
            memcpy(MGCMTBLDAT_PROCESS.KEY_1, CWIPLOTEXT.CMF_2, sizeof(CWIPLOTEXT.CMF_2)); // IS-21-05-028 Terminate Module Shift 추가건 개발
            ...
            memcpy(MGCMTBLDAT_EQ.KEY_1, CWIPLOTEXT.CMF_3, sizeof(CWIPLOTEXT.CMF_3)); // IS-21-05-028 Terminate Module Shift 추가건 개발
		...
		//25.04.03 Module Terminate V2
		if (COM_isnullspace(MWIPLOTSTS.END_RES_ID) == MP_TRUE)
        {
            TRS.add_string(list_item, "RES_ID", MWIPLOTSTS.START_RES_ID, sizeof(MWIPLOTSTS.START_RES_ID));
        }
        else
        {
        ...
        memcpy(MWIPMATDEF.FACTORY, MWIPLOTSTS.FACTORY, sizeof(MWIPLOTSTS.FACTORY));
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CWIP_View_Lot_List_Validation_Terminate(char* s_msg_code,
    TRSNode* in_node,
    TRSNode* out_node)
{
