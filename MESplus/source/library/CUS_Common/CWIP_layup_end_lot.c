...
int CWIP_LAYUP_END_LOT(char *s_msg_code,
                        TRSNode *in_node, 
                        TRSNode *out_node);
...
*******************************************************************************/
int CWIP_Layup_End_Lot(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CWIP_LAYUP_END_LOT(s_msg_code, in_node, out_node);
                        ...
                        TRSNode *out_node)
{
	...
	TRS.copy(s_to_lot_number, sizeof(s_to_lot_number), in_node, "LOT_ID");
	...
	memcpy(CWIPLOTSTR.LOT_ID, s_to_lot_number, sizeof(MWIPLOTSTS.LOT_ID));
	...
	while(i_combine_count > 0)
	{
	    ...
	    memcpy(CWIPLOTSTR.LOT_ID, MWIPLOTSTS.LOT_ID, sizeof(MWIPLOTSTS.LOT_ID));
        memcpy(CWIPLOTSTR.STRING_ID, MWIPLOTSTS_STR.LOT_ID, sizeof(MWIPLOTSTS_STR.LOT_ID));
	    /* Insert or Update */
	...
	//현재공정이 LAYUP 이면 END 시킴
	if (memcmp(MWIPLOTSTS.OPER, HQCEL_M1_LAYUP_OPER, strlen(HQCEL_M1_LAYUP_OPER)) == 0)
	{
		TRS.init_node(tran_in_node);
		TRS.init_node(tran_out_node);
...
/*******************************************************************************
    CWIP_LAYUP_END_LOT_SPLIT()
        - SOI->MES LAYUP END LOT ( LOT 이 변경되는 경우일경우 사용) .. 
    return Value
        - int : 0 (MP_TRUE)
    Arguments
        - char *s_msg_code : Error Msg Code
        ...
        - TRSNode *out_node : Output Message structure  
*******************************************************************************/
int CWIP_LAYUP_END_LOT_SPLIT(char *s_msg_code,
                        TRSNode *in_node, 
                        TRSNode *out_node)
{
	// INIT
