...
File Name   : CSUM_BatchProcess_resource_function_E1.c
...
int CSUM_BATCHPROCESS_EVENT_E2(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
/*******************************************************************************
CSUM_BatchProcess_Event_E2()
- End Lot
Return Value
- int : 0 (MP_TRUE)
Arguments
...
*******************************************************************************/
int CSUM_BatchProcess_Event_E2(TRSNode *in_node, TRSNode *out_node)
{
	...
	*/
	...
	i_ret = CSUM_BATCHPROCESS_EVENT_E2(s_msg_code, in_node, out_node);

	COM_out_msg_log_write(s_msg_code, "CSUM_BATCHPROCESS_EVENT_E2", out_node);
...
/*******************************************************************************
CSUM_BATCHPROCESS_EVENT_E2()
- Main sub function of "CSUM_BatchProcess_Event_E2" function
Return Value
...
int CSUM_BATCHPROCESS_EVENT_E2(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	...
	struct MGCMTBLDAT_TAG MGCMTBLDAT; //IS-21-06-038 E10 배치 로직 변경 개발
    ...
    LOG_head("CSUM_BATCHPROCESS_EVENT_E2");
	...
	if(TRS.get_char(in_node, "PROCSTEP") != ' ')
		MTMPRESHIS.PROCESS_FLAG = TRS.get_char(in_node, "PROCSTEP"); //MAINT 용
	...
	while(1)
	{
		...
		if (c_new_shiftday_change_flag =='Y')
		{
			...
			while(1) 
			{
				...
				//22.03.28 Factory 코드를 주지 않아 index를 타지 않음
		...
		if (c_new_record_flag == 'Y')
		{
			...
			memcpy(RSUMRESDWH.CURR_ACRO, MTMPRESHIS.NEW_PRI_STS, sizeof(RSUMRESDWH.CURR_ACRO));
			memcpy(RSUMRESDWH.CURR_GROUP, MTMPRESHIS.NEW_STS_4, sizeof(RSUMRESDWH.CURR_GROUP));
			memcpy(RSUMRESDWH.PREV_CODE, MTMPRESHIS.OLD_STS_3, sizeof(RSUMRESDWH.CURR_ACRO));  //  오타
			memcpy(RSUMRESDWH.ALID, MTMPRESHIS.TRAN_CMF_2, sizeof(RSUMRESDWH.ALID));
			memcpy(RSUMRESDWH.ALCD, MTMPRESHIS.TRAN_CMF_3, sizeof(RSUMRESDWH.ALCD));
			memcpy(RSUMRESDWH.ALTX, MTMPRESHIS.TRAN_CMF_4, sizeof(MTMPRESHIS.TRAN_CMF_4));
