/******************************************************************************'
    ...
    System      : MESplus
    Module      : CEIS
    File Name   : CEIS_end_lot_lami.c
    Description : EAPMES End Lot For LAMI Service
    ...
    Function List
        ...
        - EAPMES_END_LOT_LAMI()
            + Main sub function of EAPMES_End_Lot_LAMI function
            + Setup service main business function
        - EAPMES_End_Lot_LAMI_Validation()
            + Main sub function of EAPMES_END_LOT_LAMI function
    Detail Description
        - EAPMES_END_LOT_LAMI()
            + h_proc_step
...
int EAPMES_End_Lot_LAMI_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
...
*******************************************************************************/
int EAPMES_End_Lot_LAMI(TRSNode *in_node)
{
    ...
    i_ret = EAPMES_END_LOT_LAMI(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "EAPMES_END_LOT_LAMI", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int EAPMES_END_LOT_LAMI(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    ...
    LOG_head("EAPMES_END_LOT_LAMI");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);

    if(EAPMES_End_Lot_LAMI_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
        COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
        return MP_FALSE;
    }
    ...
    */
    ...
    for(i = 0; i < i_lot_item_count; i++)
    {
		...
		{
			memcpy(CWIPLOTRAM.PH_END_TIME, s_sys_time, sizeof(s_sys_time));
		}
		if(s_res_id[6] == 'L' && s_res_id[7] == 'F')
		{
			memcpy(CWIPLOTRAM.LF_END_TIME, s_sys_time, sizeof(s_sys_time));
		}
		if(s_res_id[6] == 'L' && s_res_id[7] == 'P')
		{
			memcpy(CWIPLOTRAM.LP_END_TIME, s_sys_time, sizeof(s_sys_time));
		}
		if(s_res_id[6] == 'L' && s_res_id[7] == 'C')
		{
			memcpy(CWIPLOTRAM.LC_END_TIME, s_sys_time, sizeof(s_sys_time));
		}
