/******************************************************************************'
    ...
    System      : MESplus
    Module      : CMMS
    File Name   : CMMS_update_calibration_institute.c
    Description : Calibration_Institute Setup function module

    MES Version : 5.3.4 ~

    Function List
        - CMMS_Update_Calibration_Institute()
            + Create/Update/Delete Calibration_Institute definition
        - CMMS_UPDATE_CALIBRATION_INSTITUTE()
            + Main sub function of CMMS_Update_Calibration_Institute function
            + Create/Update/Delete Calibration_Institute definition
        - CMMS_Update_Calibration_Institute_Validation()
            + Main sub function of CMMS_UPDATE_CALIBRATION_INSTITUTE function
            + Check the condition for create/update/delete Calibration_Institute
    Detail Description
        - CMMS_UPDATE_CALIBRATION_INSTITUTE()
            + h_proc_step
                + MP_STEP_CREATE : Create Calibration_Institute definition
                + MP_STEP_UPDATE : Update Calibration_Institute definition
                + MP_STEP_DELETE : Delete Calibration_Institute definition
...
int CMMS_Update_Calibration_Institute_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
...
*******************************************************************************/
int CMMS_Update_Calibration_Institute(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CMMS_UPDATE_CALIBRATION_INSTITUTE(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CMMS_UPDATE_CALIBRATION_INSTITUTE", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CMMS_UPDATE_CALIBRATION_INSTITUTE(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    ...
    LOG_head("CMMS_UPDATE_CALIBRATION_INSTITUTE");
    ...
    if(COM_proc_user_routine("CMMS", "CMMS_Update_Calibration_Institute",
                             MP_UPOINT_BEFORE,
                             in_node,
    ...
    */
    ...
    if(CMMS_Update_Calibration_Institute_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
        COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
        return MP_FALSE;
    }
	
	//File이 존재 시 저장 및 경로를 먼저 가져온다.
	if(TRS.get_procstep(in_node) == MP_STEP_CREATE || TRS.get_procstep(in_node) == MP_STEP_UPDATE)
	{
		...
		{	
			...
			if (m_blob.IS_NULL != 'Y')
			{
				...
				if (CMMS_set_attached_file(s_msg_code, "CMMSINSDEF", TRS.get_string(in_node, "INSTI_CODE"), TRS.get_string(in_node, "FILE_NAME"), m_blob, file_path) == MP_FALSE)
				{
					COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
					return MP_FALSE;
				}
