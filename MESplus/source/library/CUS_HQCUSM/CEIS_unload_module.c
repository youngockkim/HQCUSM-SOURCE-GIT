/******************************************************************************'
    ...
    System      : MESplus
    Module      : CEIS
    File Name   : CEIS_unload_module.c
    Description : EAPMES Unload Module Service

    MES Version : 5.3.4 ~

    Function List
        - EAPMES_Unload_Module()
            + Setup service interface function
        - EAPMES_UNLOAD_MODULE()
            + Main sub function of EAPMES_Unload_Module function
            + Setup service main business function
        - EAPMES_Unload_Module_Validation()
            + Main sub function of EAPMES_UNLOAD_MODULE function
    Detail Description
        - EAPMES_UNLOAD_MODULE()
            + h_proc_step
...
int EAPMES_Unload_Module_Validation(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
...
*******************************************************************************/
int EAPMES_Unload_Module(TRSNode *in_node)
{
    ...
    i_ret = EAPMES_UNLOAD_MODULE(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "EAPMES_UNLOAD_MODULE", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int EAPMES_UNLOAD_MODULE(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    ...
    LOG_head("EAPMES_UNLOAD_MODULE");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);

    if(EAPMES_Unload_Module_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
        COM_set_result(out_node, MP_FAIL_C, s_msg_code, MP_MSG_CATE_ERROR, TRS.get_language(in_node));
        return MP_FALSE;
    }
	...
	for (i = 0; i < icnt; i++)
	{
		...
		//LOT 의 그룹ID :  LOT_GROUP_ID 3에 처음 LOT ID 
		//                 LOT_LOCATION_1 에 순번
		//                 LOT_LOCATION_2 에 UPPER_BAR_ID(TRANSFER)
		//                 LOT_LOCATION_3 에 OWER_BAR_ID(TRANSFER)
        ...
        if(DB_error_code != DB_SUCCESS)
        {
            if(DB_error_code == DB_NOT_FOUND)
            {
				...
				memcpy(CWIPLOTRAM.UNLOAD_TIME, s_sys_time, sizeof(s_sys_time));
