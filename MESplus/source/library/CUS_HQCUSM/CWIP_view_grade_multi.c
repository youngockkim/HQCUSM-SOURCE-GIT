/******************************************************************************'
    ...
    System      : MESplus
    Module      : CWIP
    File Name   : CWIP_view_grade_multi.c
    Description : View Grade_Multi List function module

    MES Version : 5.3.4 ~

    Function List
        - CWIP_View_Grade_Multi()
            + View Grade_Multi definition List
        - CWIP_VIEW_GRADE_MULTI()
            + Main sub function of CWIP_View_Grade_Multi function
            + View Grade_Multi definition List
    Detail Description
        - CWIP_VIEW_GRADE_MULTI()
            + h_proc_step
                + 1 : View Grade_Multi definition List by Primary Key
    History
    Seq   Date        Developer      Description
    ---------------------------------------------------------------------------
    1     2025-02-04             Create by Generator
...
*******************************************************************************/
int CWIP_View_Grade_Multi(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CWIP_VIEW_GRADE_MULTI(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CWIP_VIEW_GRADE_MULTI", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CWIP_VIEW_GRADE_MULTI(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	// init
	int inx, count;
	char s_sys_time[14];
	...
	// PROCESS LOG PRINT
	LOG_head("CWIP_VIEW_GRADE_MULTI");
	...
	lot_id_list = TRS.get_list(in_node, "GRADE");
	count = TRS.get_item_count( in_node, "GRADE");
	...
	{	
		...
		if ( CWIP_SELECT_GRADE_STATUS( s_msg_code, temp_node, list_item) == MP_FALSE ) 
		{
			strcpy(s_msg_code, "WIP-0004");
...
/*******************************************************************************
	...
	Arguments
		...
		- TRSNode *out_node : Output Message structure
*******************************************************************************/
int CWIP_SELECT_GRADE_STATUS(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	// init
	struct MWIPELTSTS_TAG MWIPELTSTS;
	struct MWIPLOTSTS_TAG MWIPLOTSTS;
