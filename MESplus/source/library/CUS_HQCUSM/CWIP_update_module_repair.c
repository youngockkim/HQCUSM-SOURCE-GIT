/******************************************************************************'
    ...
    System      : MESplus
    Module      : CWIP
    File Name   : CWIP_update_module_repair.c
...
int CWIP_VIEW_MODULE_REPAIR(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int CWIP_UPDATE_MODULE_REPAIR(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int CWIP_DELETE_MODULE_REPAIR(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
...
*******************************************************************************/
int CWIP_Update_Module_Repair(TRSNode *in_node, TRSNode *out_node)
{
	...
	if(TRS.get_procstep(in_node) == '2'){
		i_ret = CWIP_VIEW_MODULE_REPAIR(s_msg_code, in_node, out_node);
	}
	else if(TRS.get_procstep(in_node) == '3'){
		i_ret = CWIP_UPDATE_MODULE_REPAIR(s_msg_code, in_node, out_node);
	}	
	else if(TRS.get_procstep(in_node) == '4'){
		i_ret = CWIP_DELETE_MODULE_REPAIR(s_msg_code, in_node, out_node);
	}
...
/*******************************************************************************
	CWIP_VIEW_MODULE_REPAIR()
		- Main sub function of "CWIP_VIEW_MODULE_REPAIR" function
		- View Lot
	Return Value
...
{ 
	...
	//[2024/12/15] Management Module Repair Precheck ADD TABBER NAME
	struct MGCMTBLDAT_TAG MGCMTBLDAT_TAB;
    ...
    LOG_head("CWIP_VIEW_MODULE_REPAIR");
	...
	TRS.set_string(out_node, "BUS_RES_ID", CWIPLOTTRC.BUS_RES_ID, sizeof(CWIPLOTTRC.BUS_RES_ID));
...
/*******************************************************************************
    CWIP_UPDATE_MODULE_REPAIR()
        - Main sub function of "CWIP_Update_Select" function
        - Create/Update/Delete Select definition
    Return Value
        - int : 1 (MP_TRUE) or 0 (MP_FALSE)M
    Arguments
        - char *s_msg_code : Error Message Code
        - TRSNode *in_node : Input Message structure
        - TRSNode *out_node : Output Message structure
...
int CWIP_UPDATE_MODULE_REPAIR(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	...
	// PROCESS LOT PRINT
    LOG_head("CWIP_UPDATE_MODULE_REPAIR");
	TRS.log_add_all_members(in_node);
	COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);
...
int CWIP_DELETE_MODULE_REPAIR(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
	...
	// PROCESS LOT PRINT
    LOG_head("CWIP_DELETE_MODULE_REPAIR");
	TRS.log_add_all_members(in_node);
	COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);
