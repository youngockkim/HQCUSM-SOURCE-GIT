/******************************************************************************'
    ...
    System      : MESplus
    Module      : CWIP
    File Name   : CRAS_multi_update_daily_work_list.c
    Description : Direct_View_Header Setup function module

    MES Version : 5.2.0

    Function List
        - CWIP_Multi_Update_Daily_Work_List()
            + Create/Update/Delete Direct_View_Header definition
        - CWIP_MULTI_UPDATE_DAILY_WORK_LIST()
            + Main sub function of CWIP_Multi_Update_Daily_Work_List function
            + Create/Update/Delete Direct_View_Header definition
        - CWIP_Multi_Update_Daily_Work_List_Validation()
            + Main sub function of CWIP_MULTI_UPDATE_DAILY_WORK_LIST function
...
*******************************************************************************/
int CRAS_Multi_Update_Daily_Work_List(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CRAS_MULTI_UPDATE_DAILY_WORK_LIST(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code,"CRAS_MULTI_UPDATE_DAILY_WORK_LIST", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CRAS_MULTI_UPDATE_DAILY_WORK_LIST(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{ 
    TRSNode **List;
    ...
    for(i=0;i<TRS.get_item_count(in_node, "WORK_LIST");i++)
    {
        ...
        if(CRAS_UPDATE_DAILY_WORK_LIST(s_msg_code, List[i], out_node)==MP_FALSE)
        {
