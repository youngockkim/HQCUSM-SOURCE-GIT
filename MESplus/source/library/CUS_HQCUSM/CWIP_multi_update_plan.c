/******************************************************************************'
    ...
    System      : MESplus
    Module      : CWIP
    File Name   : CWIP_multi_update_plan.c
    Description : Direct_View_Header Setup function module

    MES Version : 5.2.0

    Function List
        - CWIP_Multi_Update_Plan()
            + Create/Update/Delete Direct_View_Header definition
        - CWIP_MULTI_UPDATE_PLAN()
            + Main sub function of CWIP_Multi_Update_Plan function
            + Create/Update/Delete Direct_View_Header definition
        - CWIP_Multi_Update_Plan_Validation()
            + Main sub function of CWIP_MULTI_UPDATE_PLAN function
...
*******************************************************************************/
int CWIP_Multi_Update_Plan(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CWIP_MULTI_UPDATE_PLAN(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code,"CWIP_MULTI_UPDATE_PLAN", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CWIP_MULTI_UPDATE_PLAN(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{ 
    TRSNode **List;
    ...
    for(i=0;i<TRS.get_item_count(in_node, "WORK_LIST");i++)
    {
        ...
        TRS.set_nstring(List[i], "TABLE_NAME", TRS.get_string(in_node, "TABLE_NAME"));
        if(CWIP_UPDATE_PLAN(s_msg_code, List[i], out_node)==MP_FALSE)
        {
