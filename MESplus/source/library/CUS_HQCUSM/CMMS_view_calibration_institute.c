/******************************************************************************'
    ...
    System      : MESplus
    Module      : CMMS
    File Name   : CMMS_view_calibration_institute.c
    Description : View Calibration_Institute function module

    MES Version : 5.3.4 ~

    Function List
        - CMMS_View_Calibration_Institute()
            + View Calibration_Institute definition
        - CMMS_VIEW_CALIBRATION_INSTITUTE()
            + Main sub function of CMMS_View_Calibration_Institute function
            + View Calibration_Institute definition
    Detail Description
        - CMMS_VIEW_CALIBRATION_INSTITUTE()
            + h_proc_step
                + 1 : View Calibration_Institute definition  by Primary Key
    History
...
*******************************************************************************/
int CMMS_View_Calibration_Institute(TRSNode *in_node, TRSNode *out_node)
{
    ...
    i_ret = CMMS_VIEW_CALIBRATION_INSTITUTE(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "CMMS_VIEW_CALIBRATION_INSTITUTE", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int CMMS_VIEW_CALIBRATION_INSTITUTE(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    struct CMMSINSDEF_TAG CMMSINSDEF;

    LOG_head("CMMS_VIEW_CALIBRATION_INSTITUTE");
    ...
    if(COM_proc_user_routine("CMMS", "CMMS_View_Calibration_Institute",
                             MP_UPOINT_BEFORE,
                             in_node,
    ...
    */
	...
	if (COM_isspace(CMMSINSDEF.FILE_PATH, sizeof(CMMSINSDEF.FILE_PATH)) == MP_FALSE)
    {
        if(CMMS_get_attached_file(s_msg_code, out_node, CMMSINSDEF.FILE_PATH, MP_BIN_DATA_1, 'Y') == MP_FALSE)
        {
