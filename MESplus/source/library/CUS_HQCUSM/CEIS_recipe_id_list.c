...
int EAPMES_RECIPEID_LIST(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
int EAPMES_RECIPEID_LIST_HISTORY_INSERT(char *s_msg_code, TRSNode *in_node, TRSNode *out_node);
/*******************************************************************************
    EAPMES_RecipeID_List()
        - Setup service interface function
    Return Value
...
*******************************************************************************/
int EAPMES_RecipeID_List(TRSNode *in_node)
{
	...
	//HISOTRY LOG INSERT
    i_ret = EAPMES_RECIPEID_LIST_HISTORY_INSERT(s_msg_code, in_node, out_node);

    memset(s_msg_code, 0x00, MP_SIZE_MSG);
	TRS.init_node(out_node);

    i_ret = EAPMES_RECIPEID_LIST(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "EAPMES_RECIPEID_LIST", out_node);
	...
	memcpy(MGCMLAGDAT.KEY_3, "EAPMES_RecipeID_List", strlen("EAPMES_RecipeID_List"));
...
//	struct MRCPRCPDEF_TAG MRCPRCPDEF_CHK;
...
//	int	i_case, 
	...
	//0. 해당 RECOURCE 의 RECIPE 의 DELETE FLAG 'Y' 로 셋팅
	/*
	...
	TRS.copy(MRCPRCPDEF.DELETE_USER_ID, sizeof(MRCPRCPDEF.DELETE_USER_ID), in_node, "USERID");
	memcpy(MRCPRCPDEF.DELETE_TIME, s_sys_time, sizeof(s_sys_time));
	DBC_update_mrcprcpdef(101, &MRCPRCPDEF);
	if(DB_error_code != DB_SUCCESS)
    {
		//DO NOTHING
	}
	
	ITEM_LIST = TRS.get_list(in_node, "RECIPE_ITEM");
    i_list_count = TRS.get_item_count(in_node, "RECIPE_ITEM");
    ...
    for(i = 0; i < i_list_count; i++)
    {
		...
		TRS.copy(MRCPRCPDEF.RES_ID, sizeof(MRCPRCPDEF.RES_ID), in_node, "RES_ID");
		TRS.copy(MRCPRCPDEF.SUBRES_ID, sizeof(MRCPRCPDEF.SUBRES_ID), in_node, "SUB_RES_ID");
		TRS.copy(MRCPRCPDEF.RECIPE, sizeof(MRCPRCPDEF.RECIPE), ITEM_LIST[i], "RECIPE_ID");
		DBC_select_mrcprcpdef(1, &MRCPRCPDEF);
		if(DB_error_code != DB_SUCCESS)
		{
			TRS.copy(MRCPRCPDEF.RECIPE_DESC, sizeof(MRCPRCPDEF.RECIPE), ITEM_LIST[i], "RECIPE_DESCRIPTION");
			c_create_flag = 'Y';
		}
		...
		if (c_create_flag == 'Y')
		{
			...
			TRS.add_nstring(tran_in, "RECIPE_DESC", TRS.get_string(ITEM_LIST[i], "RECIPE_DESCRIPTION"));
		...
		MRCPRCPVER.RECIPE_VERSION= TRS.get_int( ITEM_LIST[i], "MES_RECIPE_VERSION");
		...
		if (c_create_flag == 'Y')
		{
			...
			TRS.add_nstring(tran_in, "PP_ID", TRS.get_string(ITEM_LIST[i], "PP_ID"));
...
int EAPMES_RECIPEID_LIST_HISTORY_INSERT(char *s_msg_code, TRSNode *in_node, TRSNode *out_node)
{
    ...
    LOG_head("EAPMES_RECIPEID_LIST_HISTORY_INSERT");
	TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_SETUP);
    ...
    for(i = 0; i < i_list_count; i++)
    {
		...
		TRS.copy(CRASRCPHIS.PP_ID, sizeof(CRASRCPHIS.PP_ID), ITEM_LIST[i], "PP_ID");
		TRS.copy(CRASRCPHIS.RECIPE_DESC, sizeof(CRASRCPHIS.RECIPE_DESC), ITEM_LIST[i], "RECIPE_DESCRIPTION");
		TRS.copy(CRASRCPHIS.CMF_5, sizeof(CRASRCPHIS.CMF_5), ITEM_LIST[i], "RECIPE_ID"); //EQ RECIPE
		TRS.copy(CRASRCPHIS.EQ_RECIPE_VERSION, sizeof(CRASRCPHIS.EQ_RECIPE_VERSION), ITEM_LIST[i], "RECIPE_VERSION"); //EQ RECIPE VERSION
		...
		//RECIPE_ID / VERSION GET -> PP ID = [RECIPE ID] + '_' + [VERSION ]
		{
			...
			TRS.set_string(ITEM_LIST[i], "RECIPE_ID", CRASRCPHIS.RECIPE_ID, sizeof(CRASRCPHIS.RECIPE_ID));
			TRS.set_string(ITEM_LIST[i], "RECIPE_VERSION", CRASRCPHIS.RECIPE_VERSION, sizeof(CRASRCPHIS.RECIPE_VERSION));
			CRASRCPHIS.MES_RECIPE_VERSION = COM_atoi(CRASRCPHIS.RECIPE_VERSION, sizeof(CRASRCPHIS.RECIPE_VERSION));
			TRS.set_int(ITEM_LIST[i], "MES_RECIPE_VERSION", CRASRCPHIS.MES_RECIPE_VERSION);
		}
		...
		memcpy(CRASRCPHIS.CMF_1, "LOAD", 4);
