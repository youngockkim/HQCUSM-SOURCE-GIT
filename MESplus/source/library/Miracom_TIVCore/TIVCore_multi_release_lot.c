/*******************************************************************************
    ...
    System      : MESplus
    Module      : INVCore
    File Name   : INVCore_multi_release_lot.c
    Description : Multi Release Lot transaction function module

    MES Version : 5.2.0.0

    Function List
        - TIV_Multi_Release_Lot()
            + Multi Release Lot
        - TIV_MULTI_RELEASE_LOT()
            + Main sub function of "TIV_Multi_Release_Lot" function
            + Multi Release Lot definition
...
int TIV_Multi_Release_Lot_Validation(char *s_msg_code,
                                       TRSNode *in_node, 
                                       TRSNode *out_node);
...
*******************************************************************************/
int TIV_Multi_Release_Lot(TRSNode *in_node,
                        TRSNode *out_node)
{
    ...
    i_ret = TIV_MULTI_RELEASE_LOT(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "TIV_MULTI_RELEASE_LOT", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure    
*******************************************************************************/
int TIV_MULTI_RELEASE_LOT(char *s_msg_code,
                             TRSNode *in_node,
                             TRSNode *out_node)
{ 
    ...
    LOG_head("TIV_Multi_Release_Lot");
    COM_log_add_field_msg(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);

    if(COM_proc_user_routine("MES_UserTIV", "TIV_Multi_Release_Lot",
                             MP_UPOINT_BEFORE,
                             in_node,
    ...
    /* Validation Check */
    if(TIV_Multi_Release_Lot_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
	...
	release_lot_in = TRS.add_node(in_node, "LOT_LIST");
    ...
    for (i = 0; i < i_lot_count; i++)
    {
		...
		TRS.add_nstring(release_lot_in, "PRC_USER", TRS.get_string(in_node, "PRC_USER"));
        TRS.set_nstring(release_lot_in, "LOT_ID", TRS.get_string(lot_list[i], "LOT_ID"));
        TRS.set_int(release_lot_in, "LAST_ACTIVE_HIST_SEQ", TRS.get_int(lot_list[i], "LAST_ACTIVE_HIST_SEQ"));
...
//                TRS.add_fieldmsg(out_node, "LAST_ACTIVE_HIST_SEQ", MP_INT, MTIVLOTSTS.LAST_ACTIVE_HIST_SEQ);
        ...
        TRS.set_int(release_lot_in, "FLOW_SEQ_NUM", MWIPLOTSTS.FLOW_SEQ_NUM);
        TRS.set_string(release_lot_in, "OPER", MWIPLOTSTS.OPER, sizeof(MWIPLOTSTS.OPER));*/


        if(TIV_RELEASE_LOT(s_msg_code, release_lot_in, out_node) == MP_FALSE)
        {
            TRS.free_node(release_lot_in);
            return MP_FALSE;
        }
...
/*******************************************************************************
    TIV_Multi_Release_Lot_Validation()
        - Validation Check sub function of "TIV_Multi_Release_Lot" function
