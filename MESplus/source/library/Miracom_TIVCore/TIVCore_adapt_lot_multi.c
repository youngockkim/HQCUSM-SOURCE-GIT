...
int TIV_Adapt_Lot_Multi_Validation(char *s_msg_code,
                               TRSNode *in_node, 
                               TRSNode *out_node);
...
*******************************************************************************/
int TIV_Adapt_Lot_Multi(TRSNode *in_node, 
                  TRSNode *out_node)
{
    ...
    i_ret = TIV_ADAPT_LOT_MULTI(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "TIV_ADAPT_LOT_MULTI", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - Cmn_Out_Tag *Cmn_Out : Output Message structure
*******************************************************************************/
int TIV_ADAPT_LOT_MULTI(char *s_msg_code,
                       TRSNode *in_node, 
                       TRSNode *out_node)

{
    ...
    LOG_head("TIV_ADAPT_LOT_MULTI");
    TRS.log_add_all_members(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);

    if(COM_proc_user_routine("MES_UserTIV", "TIV_Adapt_Lot_Multi",
                             MP_UPOINT_BEFORE,
                             in_node,
    ...
    /*' Validation Check*/
    if(TIV_Adapt_Lot_Multi_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
	...
	for(i = 0; i < i_lot_count; i++)
	{
		...
		if(TIV_ADAPT_LOT(s_msg_code, inv_lot_list[i], out_node) == FALSE)
		{
			return MP_FALSE;
		}
    }
...
/*******************************************************************************
    TIV_Adapt_Lot_Multi_Validation()
        - Validation Check sub function of "TIV_Transfer_Lot" function
    Return Value
        - int : 1 (MP_TRUE) / 0 (MP_FALSE)
    Arguments
