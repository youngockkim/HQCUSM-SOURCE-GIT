/*******************************************************************************
    ...
    System      : MESplus
    Module      : TIVCore
    File Name   : TIVCore_multi_merge_lot.c
    Description : Multi Merge Lot transaction function module
    ...
    Function List
        - TIV_Multi_Merge_Lot()
            + Multi Merge Lot
        - TIV_MULTI_MERGE_LOT()
            + Main sub function of "TIV_Multi_Merge_Lot" function
...
int TIV_Multi_Merge_Lot_Validation(char *s_msg_code,
                                       TRSNode *in_node, 
                                       TRSNode *out_node);
...
*******************************************************************************/
int TIV_Multi_Merge_Lot(TRSNode *in_node,
                        TRSNode *out_node)
{
    ...
    i_ret = TIV_MULTI_MERGE_LOT(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "TIV_MULTI_MERGE_LOT", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure    
*******************************************************************************/
int TIV_MULTI_MERGE_LOT(char *s_msg_code,
                             TRSNode *in_node,
                             TRSNode *out_node)
{ 
    ...
    LOG_head("TIV_Multi_Merge_Lot");
    COM_log_add_field_msg(in_node);
    COM_log_write(MP_LOG_INFORMATION, MP_LOG_E_LOGIC, MP_LOG_CATE_TRANS);

    if(COM_proc_user_routine("MES_UserTIV", "TIV_Multi_Merge_Lot",
                             MP_UPOINT_BEFORE,
                             in_node,
    ...
    /* Validation Check */
    if(TIV_Multi_Merge_Lot_Validation(s_msg_code, in_node, out_node) == MP_FALSE)
    {
    ...
    for (i = 0; i < i_lot_count; i++)
    {
		...
		TRS.set_nstring(lot_list[i], "OPER", TRS.get_string(lot_list[i], "FROM_OPER"));

        


        if(TIV_MERGE_LOT(s_msg_code, lot_list[i], out_node) == MP_FALSE)
        {
            //TRS.free_node(merge_lot_in);
            return MP_FALSE;
        }
...
/*******************************************************************************
    TIV_Multi_Merge_Lot_Validation()
        - Validation Check sub function of "TIV_Multi_Merge_Lot" function
