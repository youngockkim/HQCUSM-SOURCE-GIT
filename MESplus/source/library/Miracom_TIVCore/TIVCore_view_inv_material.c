/*******************************************************************************
    ...
    System      : MESplus
    Module      : INV
    File Name   : TIV_View_Inv_Material.c
    Description : Create/Update/Delete Material definition

    MES Version : 4.0.0

    Function List
        - TIV_View_Inv_Material()
            + View Material information
        - TIV_VIEW_TIV_MATERIAL()
            + Main sub function of "TIV_View_Inv_Material" function
...
*******************************************************************************/
int TIV_View_Inv_Material(TRSNode *in_node, 
                      TRSNode *out_node)
{
    ...
    i_ret = TIV_VIEW_INV_MATERIAL(s_msg_code, in_node, out_node);

    COM_out_msg_log_write(s_msg_code, "TIV_VIEW_TIV_MATERIAL", out_node);
...
/*******************************************************************************
    ...
    Arguments
        ...
        - TRSNode *out_node : Output Message structure
*******************************************************************************/
int TIV_VIEW_INV_MATERIAL(char *s_msg_code,
                           TRSNode *in_node, 
                           TRSNode *out_node)
{
    ...
    LOG_head("TIV_VIEW_INV_MATERIAL");
    ...
    if(COM_proc_user_routine("MES_UserTIV", "TIV_View_Inv_Material",
                             MP_UPOINT_BEFORE,
                             in_node,
    ...
    if(TRS.get_int(in_node, "MAT_VER") < 1 && TRS.get_procstep(in_node)!='3')
    {
        strcpy(s_msg_code, "INV-0001");
    ...
    DBC_select_mtivmatdef(i_step, &MTIVMATDEF);
    if(DB_error_code != DB_SUCCESS) 
    {
